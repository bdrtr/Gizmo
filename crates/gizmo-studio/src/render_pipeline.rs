...
pub fn execute_render_pipeline(world: &mut World, state: &StudioState, encoder: &mut wgpu::CommandEncoder, view: &wgpu::TextureView, renderer: &mut gizmo::renderer::Renderer, _light_time: f32) {
        ...
        // --- 2. ANA RENDER PASS (HDR Offscreen Texture'a çiz) ---
        {
            ...
            for batch in &flat_batches {
                if batch.is_transparent || batch.is_double_sided || batch.is_skybox { continue; } // Şeffafları, Skybox'ı ve çift yönlüleri atla
                if batch.start_instance >= 100_000 { continue; }
                let safe_end = std::cmp::min(batch.end_instance, 100_000);
            ...
            for batch in &flat_batches {
                if batch.is_transparent || !batch.is_double_sided || batch.is_skybox { continue; }
                if batch.start_instance >= 100_000 { continue; }
                let safe_end = std::cmp::min(batch.end_instance, 100_000);
            ...
            for batch in &flat_batches {
                if !batch.is_skybox { continue; } // Sadece Skybox'u çiz
                if batch.start_instance >= 100_000 { continue; }
                let safe_end = std::cmp::min(batch.end_instance, 100_000);
