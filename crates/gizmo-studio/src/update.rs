use gizmo::prelude::*;
use gizmo::editor::{EditorState, BuildTarget};
...
pub fn update_studio(world: &mut World, state: &mut StudioState, dt: f32, input: &Input) {
    ...
    if let Some(mut editor_state) = world.remove_resource::<EditorState>() {
        ...
        if let Some(ndc) = editor_state.mouse_ndc {
            ...
            if let (Some(transforms), Some(cameras)) = (world.borrow::<Transform>(), world.borrow::<gizmo::renderer::components::Camera>()) {
                if let (Some(t), Some(cam)) = (transforms.get(state.editor_camera), cameras.get(state.editor_camera)) {
        ...
        if let Some(asset_path) = editor_state.spawn_asset_request.take() {
            ...
            if let Some(ndc) = editor_state.spawn_asset_position {
                ...
                if let Some(ray) = studio_input::build_ray(world, state.editor_camera, ndc.x, ndc.y, aspect, 1.0) {
                    ...
                    if let (Some(colliders), Some(transforms)) = (world.borrow::<Collider>(), world.borrow::<Transform>()) {
                        for i in 0..colliders.dense.len() {
                            ...
                            if let Some(t) = (*transforms).get(id) {
                                ...
                                let local_ray = gizmo::math::Ray::new(local_ray_origin, local_ray_dir);
                    ...
                    if closest_t < std::f32::MAX {
                        ...
                        } else { // Işık yukarı bakıyorsa 15 birim öteye atalım
            ...
            if asset_path.ends_with(".prefab") {
                ...
                editor_state.log_error(&format!("Sadece prefab dosyaları sahnede spawn edilebilir: {}", asset_path));
            }
        }
        ...
        if editor_state.build_request {
            ...
            editor_state.is_building.store(true, std::sync::atomic::Ordering::SeqCst);
            ...
            std::thread::spawn(move || {
                ...
                let (target_triple, exe_name, target_label) = match build_target {
                    BuildTarget::Native  => (None, if cfg!(windows) { "demo.exe" } else { "demo" }, "Native"),
                    BuildTarget::Linux   => (Some("x86_64-unknown-linux-gnu"),  "demo",     "Linux (ELF)"),
                    BuildTarget::Windows => (Some("x86_64-pc-windows-gnu"),     "demo.exe", "Windows (.exe)"),
                    BuildTarget::MacOs   => (Some("x86_64-apple-darwin"),        "demo",     "macOS"),
                };

                log(&format!("== [Adım 1/3] Gizmo Build Başlıyor — Hedef: {} ==", target_label));
                    ...
                    Ok(mut child) => {
                        ...
                        if status.success() {
                            ...
                            let src_base = if let Some(triple) = target_triple {
                                std::path::PathBuf::from("target").join(triple).join("release")
                            } else {
                            ...
                            if let Err(e) = std::fs::copy(&src_exe, &dst_exe) {
                                log(&format!("HATA: Executable kopyalanamadı ({:?}): {}", src_exe, e));
                            } else {
        ...
        if input.is_key_pressed(gizmo::winit::keyboard::KeyCode::ControlLeft as u32) {
            ...
            if ctrl_pressed && input.is_key_just_pressed(gizmo::winit::keyboard::KeyCode::KeyD as u32) {
                // Şimdilik sadece ilk seçili objeyi kopyalasın
                if let Some(&first) = editor_state.selected_entities.iter().next() {
                    editor_state.duplicate_request = Some(first);
                }
            }
        } else {
            // Kısayol: Delete (Sil)
            if input.is_key_just_pressed(gizmo::winit::keyboard::KeyCode::Delete as u32) {
                if let Some(&first) = editor_state.selected_entities.iter().next() {
                    editor_state.despawn_request = Some(first);
                    // İleriki devfazlarda tüm döngüyü sil moduna alacağız
                }
                ...
                if !editor_state.selected_entities.is_empty() {
                    if let Some(transforms) = world.borrow::<Transform>() {
                        ...
                        if count > 0.0 {
                            ...
                            if let (Some(mut t_mut), Some(mut cam_mut)) = (world.borrow_mut::<Transform>(), world.borrow_mut::<gizmo::renderer::components::Camera>()) {
                                if let (Some(cam_t), Some(cam)) = (t_mut.get_mut(state.editor_camera), cam_mut.get_mut(state.editor_camera)) {
            ...
            if !changes.is_empty() {
                for changed_path in changes {
                    ...
                    if is_script {
                        ...
                        editor_state.log_warning(&format!("🔥 Shader Hot-Reload iskitleniyor: {}", path_str));
        ...
        if editor_state.is_playing() {
            ...
            if let Some(mut engine) = world.remove_resource::<gizmo::scripting::ScriptEngine>() {
                ...
                let unhandled_commands = engine.flush_commands(world);
                ...
                if let Some(scripts) = world.borrow::<gizmo::scripting::Script>() {
                    ...
                    for &entity_id in &scripts.entity_dense {
                    ...
                    for (entity_id, path) in entity_calls {
                        ...
                        if let Err(e) = engine.update_entity(entity_id, dt) {
        ...
        if let Some((ent_id, comp_name)) = editor_state.add_component_request.take() {
            if let Some(ent) = world.get_entity(ent_id) {
                match comp_name.as_str() {
                    ...
                    "RigidBody" => world.add_component(ent, gizmo::physics::RigidBody::new(1.0, 0.5, 0.5, true)),
                    "Collider" => world.add_component(ent, gizmo::physics::Collider::new_aabb(1.0, 1.0, 1.0)),
                    "Camera" => world.add_component(ent, gizmo::renderer::components::Camera::new(60.0_f32.to_radians(), 0.1, 1000.0, 0.0, 0.0, false)),
                    "PointLight" => world.add_component(ent, gizmo::renderer::components::PointLight::new(Vec3::new(1., 1., 1.), 1.0)),
                    "Material" => {
                        let white_tex = world.get_resource::<DebugAssets>().map(|a| a.white_tex.clone());
                        if let Some(tex) = white_tex {
                            world.add_component(ent, gizmo::prelude::Material::new(tex));
                        }
                    },
                    "Script" => world.add_component(ent, gizmo::scripting::Script::new("scripts/new_script.lua")),
                    "ParticleEmitter" => world.add_component(ent, gizmo::renderer::components::ParticleEmitter::new()),
                    "AudioSource" => world.add_component(ent, gizmo_audio::AudioSource { sound_name: "".to_string(), is_3d: true, max_distance: 100.0, volume: 1.0, pitch: 1.0, loop_sound: false, _internal_sink_id: None }),
                    "Terrain" => {
                        world.add_component(ent, gizmo::renderer::components::Terrain {
        ...
        // --- ENTITY SİLME ---
        if let Some(ent_id) = editor_state.despawn_request.take() {
        ...
        if let Some(kind) = editor_state.spawn_request.take() {
            let pending_assets = world.get_resource::<DebugAssets>()
            ...
            if let Some((cube_mesh, white_tex)) = pending_assets {
                ...
                match kind.as_str() {
                    "Cube" => {
                        ...
                        world.add_component(e, gizmo::prelude::Material::new(white_tex).with_pbr(
                            gizmo::math::Vec4::new(0.8, 0.8, 0.8, 1.0), 0.5, 0.0
                        ));
                    ...
                    "Sphere" => {
                        ...
                        world.add_component(e, cube_mesh);
                        world.add_component(e, gizmo::prelude::Material::new(white_tex).with_pbr(
                            gizmo::math::Vec4::new(0.4, 0.6, 1.0, 1.0), 0.2, 0.0
                        ));
                        world.add_component(e, gizmo::physics::Collider::new_sphere(1.0));
                        editor_state.log_info("Yeni küre oluşturuldu.");
                    }
                    _ => {
                        world.add_component(e, gizmo::core::component::EntityName("Boş Entity".to_string()));
        ...
        if let Some(ent_id) = editor_state.toggle_visibility_request.take() {
            if let Some(ent) = world.get_entity(ent_id) {
                let currently_hidden = world.borrow::<gizmo::core::component::IsHidden>()
        ...
        if let Some((child_id, new_parent_id)) = editor_state.reparent_request.take() {
            ...
            if let Some(child_ent) = world.get_entity(child_id) {
                ...
                editor_state.log_info(&format!("Entity {} parent {} olarak ayarlandı.", child_id, new_parent_id));
            }
        }
    ...
    let camera_speed = world.get_resource::<EditorState>()
