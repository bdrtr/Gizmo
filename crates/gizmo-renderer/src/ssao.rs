...
impl SsaoState {
    pub fn new(
        ...
        let apply_bind_group = mk_apply_bg(
            device,
            &apply_bgl,
            &ao_blurred_view,
            &gbuf_sampler,
    ...
    pub fn resize(
        ...
        self.apply_bind_group = mk_apply_bg(
            device,
            &self.apply_bgl,
            &ao_blurred_view,
            &self.gbuf_sampler,
...
fn build_kernel() -> SsaoKernel {
    ...
    let mut kernel = SsaoKernel {
        samples: [[0.0; 4]; KERNEL_SIZE],
    };
...
fn mk_ssao_gbuf_bg(
    ...
    device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            ...
            wgpu::BindGroupEntry {
                binding: 3,
                resource: wgpu::BindingResource::Sampler(gbuf_sampler),
            },
            wgpu::BindGroupEntry {
                binding: 4,
...
fn mk_blur_pipeline(device: &wgpu::Device, bgl: &wgpu::BindGroupLayout) -> wgpu::RenderPipeline {
    let shader = load_shader(
        device,
        "demo/assets/shaders/ssao_blur.wgsl",
...
fn mk_apply_pipeline(device: &wgpu::Device, bgl: &wgpu::BindGroupLayout) -> wgpu::RenderPipeline {
    let shader = load_shader(
        device,
        "demo/assets/shaders/ssao_apply.wgsl",
        include_str!("shaders/ssao_apply.wgsl"),
        "SSAO Apply Shader",
    );
    let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
