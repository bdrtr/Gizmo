...
pub fn build_scene_pipelines(device: &wgpu::Device) -> SceneState {
    ...
    let create_main =
        |sm: &wgpu::ShaderModule, label: &str, depth_write: bool, cull: Option<wgpu::Face>| {
    ...
    let unlit_pipeline = create_main(
        &unlit_shader,
        "Unlit Pipeline",
        true,
        Some(wgpu::Face::Back),
    ...
    let water_pipeline = create_main(
        &water_shader,
        "Water Pipeline",
        true,
        Some(wgpu::Face::Back),
...
pub fn rebuild_pipelines(renderer: &mut crate::Renderer) {
    ...
    let unlit_shader = load(
        "demo/assets/shaders/unlit.wgsl",
        include_str!("unlit.wgsl"),
        "Unlit Shader",
    );
    let water_shader = load(
        "demo/assets/shaders/water.wgsl",
        include_str!("water.wgsl"),
        "Water Shader",
    );
    let shadow_shader = load(
        "demo/assets/shaders/shadow.wgsl",
        include_str!("shadow.wgsl"),
        "Shadow Shader",
    );
    let post_shader = load(
    ...
    let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        ...
        bind_group_layouts: &[
            ...
            &renderer.scene.shadow_bind_group_layout,
    ...
    renderer.scene.shadow_pipeline =
        device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Shadow Pipeline"),
            layout: Some(&shadow_layout),
            vertex: wgpu::VertexState {
