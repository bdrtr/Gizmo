...
pub fn register_audio_api(lua: &Lua, command_queue: Arc<CommandQueue>) -> Result<(), LuaError> {
    ...
    // === SES ÇALMA ===
    {
        ...
        audio_table.set("play", lua.create_function(move |_, sound_name: String| {
    ...
    {
        let cq = command_queue.clone();
        audio_table.set("play_3d", lua.create_function(move |_, (sound_name, x, y, z): (String, f32, f32, f32)| {
    ...
    {
        let cq = command_queue.clone();
        audio_table.set("stop", lua.create_function(move |_, sound_name: String| {
