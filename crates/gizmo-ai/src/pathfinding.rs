...
impl Ord for AStarNode {
    fn cmp(&self, other: &Self) -> Ordering {
        other.cost.cmp(&self.cost) // Ters çevirildi
...
pub fn find_path(grid: &NavGrid, start_world: Vec3, end_world: Vec3) -> Option<Vec<Vec3>> {
    ...
    while let Some(current_node) = open_set.pop() {
        ...
        for neighbor in grid.get_neighbors(current) {
            ...
            let move_cost = if neighbor.x != current.x && neighbor.z != current.z { 14 } else { 10 };
            ...
            if tentative_g < *g_score.get(&neighbor).unwrap_or(&u32::MAX) {
                ...
                open_set.push(AStarNode { pos: neighbor, cost: f_score });
            }
        }
    }
