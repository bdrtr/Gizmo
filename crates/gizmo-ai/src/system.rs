...
pub fn ai_navigation_system(world: &World, dt: f32) {
    ...
    let agent_entities = agents.entity_dense.clone(); 
    
    // Iterasyon
    for &e in &agent_entities {
        let agent_id = e;
        
        let t = if let Some(t) = transforms.get(agent_id) { t } else { continue };
        let v = if let Some(v) = velocities.get_mut(agent_id) { v } else { continue };
        let agent = agents.get_mut(agent_id).unwrap();

        if agent.target.is_none() {
            // Yavaşla ve dur
            v.linear = steering::arrive(t.position, t.position, v.linear, agent.max_speed, agent.max_force, 1.0) * dt;
            continue;
        }
        ...
        let mut needs_recalc = agent.current_path_index >= agent.path.len() || agent.path_recalc_timer <= 0.0;
        ...
        if agent.current_path_index < agent.path.len() {
            ...
            if dist_to_node < agent.reach_radius || (v.linear.length() < 0.2 && dist_to_node < agent.reach_radius * 2.5) {
                agent.current_path_index += 1; // O(1) — remove(0) yerine indeks ilerlet
                if agent.current_path_index >= agent.path.len() {
                    // Yolu tamamladı
                    v.linear += steering::arrive(t.position, target_pos, v.linear, agent.max_speed, agent.max_force, agent.reach_radius * 2.0) * dt;
                    continue;
                } else {
            ...
            let steering_force = if remaining_nodes == 1 {
                steering::arrive(t.position, next_node, v.linear, agent.max_speed, agent.max_force, agent.reach_radius * 2.0)
            } else {
                steering::seek(t.position, next_node, v.linear, agent.max_speed, agent.max_force)
            };
            
            v.linear += steering_force * dt;
        } else {
             // Yol yoksa, en azından hedef doğrultusunda doğrudan gitmeyi dene
             let steering_force = steering::arrive(t.position, target_pos, v.linear, agent.max_speed, agent.max_force, agent.reach_radius * 2.0);
             v.linear += steering_force * dt;
        }
