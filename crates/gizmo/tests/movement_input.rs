...
const EXCEPTIONS: &[(&str, &str)] = &[
    ...
    (
        "demo/src/bin/keyboard_input.rs",
        "the key is the SUBJECT, not a direction: this demo watches one key (A) to show the \
         three edges apart — pressed, just-pressed, just-released — and nothing in it moves. \
         Routing it through `move_axis` would blend the very edges the demo exists to separate, \
         and the stick has nothing to contribute to a key-state readout.",
    ),
    (
