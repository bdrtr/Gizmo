...
use crate::renderer::{Renderer, components::{Camera, MeshRenderer, Material, Mesh}};
...
pub fn default_render_pass(
    ...
    if let (Some(cameras), Some(transforms)) = (world.borrow::<Camera>(), world.borrow::<Transform>()) {
        if let Some(active_cam) = cameras.entity_dense.first() {
    ...
    let scene_uniform_data = crate::renderer::gpu_types::SceneUniforms {
        ...
        lights: [crate::renderer::gpu_types::LightData { position: [0.0; 4], color: [0.0; 4] }; 10],
    ...
    if let Some(q) = world.query_ref_ref_ref::<Mesh, Transform, Material>() {
        for (e, mesh, trans, mat) in q.iter() {
            if renderers.as_ref().map_or(true, |r| r.get(e).is_none()) { continue; }
            ...
            let instance_data = crate::renderer::gpu_types::InstanceRaw {
                ...
                roughness: mat.roughness, metallic: mat.metallic, unlit: mat.unlit,
                _padding: 0.0,
            };
    ...
    if !instances.is_empty() {
        renderer.queue.write_buffer(&renderer.scene.instance_buffer, 0, bytemuck::cast_slice(&instances));
    }
