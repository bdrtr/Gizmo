...
fn segment_ease(c: &mut Criterion) {
    ...
    c.bench_function("segment_ease", |b| {
        let mut t = 0;
...
fn curve_position(c: &mut Criterion) {
    ...
    fn bench_curve<M: Measurement, P: VectorSpace>(
        ...
        group.bench_with_input(BenchmarkId::from_parameter(name), &curve, |b, curve| {
            b.iter(|| curve.position(core::hint::black_box(0.5)));
        });
    }

    let mut group = c.benchmark_group("curve_position");
...
fn curve_iter_positions(c: &mut Criterion) {
    ...
    c.bench_function("curve_iter_positions", |b| {
        b.iter(|| {
            for x in bezier.iter_positions(core::hint::black_box(100)) {
