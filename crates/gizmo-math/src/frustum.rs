...
impl Frustum {
    ...
    pub fn from_matrix(vp: &Mat4) -> Self {
        ...
        // Near Plane
        let _near = Plane::new(m41 + m31, m42 + m32, m43 + m33, m44 + m34); // Note: For WebGPU / API depth [0, 1] usually just m31, m32... But we'll use standard m4+m3 for [-1, 1]. In WGPU it depends on configuration. GIZMO engine defaults to wgpu depth format. Let's just store classic.`m31`, `m32`, `m33`, `m34` for Near if depth is 0..1
