...
pub fn solve_constraints(
    ...
    if joint_world.joints.is_empty() { return; }
    ...
    for _iter in 0..iterations {
        for joint in &joint_world.joints {
            ...
            let (inv_mass_a, inv_inertia_a) = rbs.get(joint.entity_a).map_or((0.0, Vec3::ZERO), |rb| {
                if rb.mass > 0.0 { (1.0 / rb.mass, rb.inverse_inertia) } else { (0.0, Vec3::ZERO) }
            });
            let (inv_mass_b, inv_inertia_b) = rbs.get(joint.entity_b).map_or((0.0, Vec3::ZERO), |rb| {
                if rb.mass > 0.0 { (1.0 / rb.mass, rb.inverse_inertia) } else { (0.0, Vec3::ZERO) }
            });
            ...
            match &joint.kind {
                JointKind::BallSocket => {
                    ...
                    for &axis in &axes {
                        ...
                        let eff_mass_inv = inv_mass_a + inv_mass_b + r_a_cross_n_i.dot(r_a_cross_n) + r_b_cross_n_i.dot(r_b_cross_n);
                    ...
                    if let Some(v_a) = vels.get_mut(joint.entity_a) { v_a.linear = va_lin; v_a.angular = va_ang; }
                    if let Some(v_b) = vels.get_mut(joint.entity_b) { v_b.linear = vb_lin; v_b.angular = vb_ang; }
                }
                JointKind::Fixed { relative_rotation } => {
                    let diff = pos_b - pos_a;
                    let correction = diff * (beta / dt) * joint.stiffness;
                    if let Some(v_a) = vels.get_mut(joint.entity_a) { v_a.linear += correction * (inv_mass_a / total_inv_mass); }
                    if let Some(v_b) = vels.get_mut(joint.entity_b) { v_b.linear -= correction * (inv_mass_b / total_inv_mass); }
                    ...
                    if angle.abs() > 0.001 {
                        ...
                        if let Some(v_a) = vels.get_mut(joint.entity_a) { v_a.angular -= angular_correction * 0.5; }
                        if let Some(v_b) = vels.get_mut(joint.entity_b) { v_b.angular += angular_correction * 0.5; }
                    }
                }
                JointKind::Distance { length } => {
                    ...
                    if let Some(v_a) = vels.get_mut(joint.entity_a) { v_a.linear -= impulse * inv_mass_a; }
                    if let Some(v_b) = vels.get_mut(joint.entity_b) { v_b.linear += impulse * inv_mass_b; }
                }
                JointKind::Spring { rest_length, spring_constant } => {
                    ...
                    if let Some(v_a) = vels.get_mut(joint.entity_a) { v_a.linear += total_force * inv_mass_a * dt; }
                    if let Some(v_b) = vels.get_mut(joint.entity_b) { v_b.linear -= total_force * inv_mass_b * dt; }
                }
                JointKind::Hinge { axis, min_angle, max_angle } => {
                    ...
                    if *min_angle > f32::NEG_INFINITY || *max_angle < f32::INFINITY {
                        ...
                        if signed_angle < *min_angle {
                            let angular_correction = *axis * (*min_angle - signed_angle) * (beta / dt);
                            if let Some(v_b) = vels.get_mut(joint.entity_b) { v_b.angular += angular_correction; }
                        } else if signed_angle > *max_angle {
                            let angular_correction = *axis * (signed_angle - *max_angle) * (beta / dt);
                            if let Some(v_b) = vels.get_mut(joint.entity_b) { v_b.angular -= angular_correction; }
                        }
                    }
                }
