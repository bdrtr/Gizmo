...
pub fn compute_convex_hull(points: &[Vec3]) -> ConvexHull {
    ...
    for i in 1..pts.len() {
        let p = pts[i];
        if p.x < pts[min_x].x { min_x = i; }
        if p.x > pts[max_x].x { max_x = i; }
        if p.y < pts[min_y].y { min_y = i; }
        if p.y > pts[max_y].y { max_y = i; }
        if p.z < pts[min_z].z { min_z = i; }
        if p.z > pts[max_z].z { max_z = i; }
    }
    ...
    if max_dist_sq < 1e-8 {
        return ConvexHull { vertices: vec![pts[0]], faces: vec![] };
    }
    ...
    if max_dist_line < 1e-8 {
        return ConvexHull { vertices: vec![pts[p0], pts[p1]], faces: vec![] };
    }
    ...
    if max_dist_plane < 1e-8 {
        ...
        dedup.dedup_by(|a, b| (a.x - b.x).abs() < 1e-4 && (a.y - b.y).abs() < 1e-4 && (a.z - b.z).abs() < 1e-4);
        return ConvexHull { vertices: dedup, faces: vec![] };
    }
