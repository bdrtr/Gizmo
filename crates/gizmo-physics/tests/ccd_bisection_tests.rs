...
fn test_ccd_no_false_positive() {
    ...
    let va = world.borrow::<Velocity>().get(a.id()).unwrap().clone();
    let vb = world.borrow::<Velocity>().get(b.id()).unwrap().clone();
...
fn test_ccd_no_penetration_smoke() {
    ...
    let ta = world.borrow::<Transform>().get(a.id()).unwrap().clone();
    let va = world.borrow::<Velocity>().get(a.id()).unwrap().clone();
