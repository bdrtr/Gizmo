...
fn test_tunneling_without_ccd() {
    ...
    assert!(
        t.position.x > 5.0,
        "Tünelleme gerçekleşmeliydi ancak mermi takıldı! Pos: {}",
        t.position.x
    );
}
...
fn test_tunneling_prevention_with_ccd() {
    ...
    assert!(
        t.position.x < 0.0,
        "CCD başarısız oldu, obje duvardan geçti veya içine girdi! Pos: {}",
        t.position.x
    );
    assert!(
        (t.position.x - -0.6).abs() < 0.1,
        "CCD objeyi tam duvarın dibinde durduramadı! Pos: {}",
        t.position.x
    );
    ...
    assert!(
        v.linear.x < 1.0,
        "Çarpışma hızı kesmedi! Kalan X hız: {}",
        v.linear.x
    );
}
