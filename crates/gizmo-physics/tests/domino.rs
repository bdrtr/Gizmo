...
fn domino_chain_reaction() {
    ...
    world.add_component(floor, Collider { shape: ColliderShape::Aabb(Aabb { half_extents: Vec3::new(10.0, 0.5, 120.0) }) });
    ...
    for i in 0..num_dominoes {
        ...
        world.add_component(domino, Collider { shape: ColliderShape::Aabb(Aabb { half_extents: Vec3::new(dx, dy, dz) }) });
    ...
    ball_rb.inverse_inertia = Vec3::new(1.0 / inertia, 1.0 / inertia, 1.0 / inertia);
    
    world.add_component(heavy_ball, ball_rb);
    world.add_component(heavy_ball, Collider { shape: ColliderShape::Sphere(Sphere { radius: 0.5 }) });
    
    // Güçlü bir itme kuvveti verelim
    world.add_component(heavy_ball, Velocity::new(Vec3::new(0.0, 0.0, 15.0)));
    ...
    println!("Zincirleme Reaksiyon Başarılı! Son domino düştü. (Son Yüksekliği: {:.2})", last_transform.position.y);
}
