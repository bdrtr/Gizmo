...
use gizmo_physics::shape::{Aabb, Capsule, ColliderShape, Sphere};
...
fn test_aabb_vs_aabb_collision() {
    ...
    assert!(
        ...
        "Kutular birbiri içine geçmiş olmalı!"
    );
    assert!(
        (manifold.penetration - 0.5).abs() < 0.001,
        "Penetrasyon 0.5 olmalı"
    );
    assert!(
        (manifold.normal.x - -1.0).abs() < 0.001 || (manifold.normal.x - 1.0).abs() < 0.001,
        "Normal X ekseninde olmalı"
    );
}
...
fn test_capsule_vs_floor_aabb() {
    ...
    assert!(
        (manifold.normal.y - -1.0).abs() < 0.001,
        "Normal A'dan B'ye doğru, yani karakterden zemine doğru Y=-1 bakmalı"
    );
}
