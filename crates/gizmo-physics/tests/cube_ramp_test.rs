...
fn test_cube_ramp_collision() {
    ...
    println!("Ramp Pos: {:?}, Cube Pos: {:?}", ramp_t.position, cube_t.position);
    
    let (is_colliding, simplex) = gizmo_physics::gjk::gjk_intersect(
        &cube_col.shape, cube_t.position, cube_t.rotation,
        &ramp_col.shape, ramp_t.position, ramp_t.rotation
    );
    ...
    if is_colliding {
        ...
        println!("Manifold Normal: {:?}, Penetration: {}", manifold.normal, manifold.penetration);
...
fn test_cube_edge_collision() {
    ...
    if is_colliding {
        ...
        println!("Edge Normal: {:?}, Edge Penetration: {}", manifold.normal, manifold.penetration);
...
fn test_box_drop_straight() {
    ...
    let ramp_vertices = vec![
        Vec3::new(-10.0, -0.5, -10.0), Vec3::new( 10.0, -0.5, -10.0),
        Vec3::new( 10.0,  0.5, -10.0), Vec3::new(-10.0,  0.5, -10.0),
        Vec3::new(-10.0, -0.5,  10.0), Vec3::new( 10.0, -0.5,  10.0),
        Vec3::new( 10.0,  0.5,  10.0), Vec3::new(-10.0,  0.5,  10.0),
    ];
    ...
    if is_colliding {
        ...
        println!("Normal: {:?}, Penetration: {}", manifold.normal, manifold.penetration);
        println!("Contact Points: {:?}", manifold.contact_points);
        println!("--------------------------\n");
    }
}
