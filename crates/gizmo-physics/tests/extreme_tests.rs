...
fn test_extreme_quantum_stack_explosion() {
    ...
    for i in 0..num_objects {
        let t = world.borrow::<Transform>().unwrap().get(entities[i].id()).unwrap().clone();
        ...
        assert!(dist > 0.1, "O kadar iç içe obje birbirini dışarı kusmalıydı! Hala merkezde: {}", dist);
    }
}
...
fn test_extreme_spring_snap() {
    ...
    let joint = Joint {
        ...
        kind: JointKind::Spring { rest_length: 100.0, spring_constant: 10000.0 }, // Sadece 100m olması genek yay!
        anchor_a: Vec3::ZERO,
        anchor_b: Vec3::ZERO,
    ...
    assert!(v_sat.linear.x < -1000.0, "Büyük gerilim çözümsüz fırlatması gerekirdi, gücü yetersiz. Hız: {}", v_sat.linear.x);
    // Ama NaN olmamalı! Sonsuz olmamalı!
    assert!(!v_sat.linear.x.is_nan(), "Fizik motoru NaN (Not a Number) patlaması yaşadı!");
    assert!(v_sat.linear.x > f32::NEG_INFINITY, "Hız sonsuzluğa (Infinity) kaydı!");
}
