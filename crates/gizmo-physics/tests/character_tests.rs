...
fn test_character_gravity_and_grounding() {
    ...
    let t2 = world.borrow::<Transform>().unwrap().get(char_entity.id()).unwrap().clone();
    let cc2 = world.borrow::<CharacterController>().unwrap().get(char_entity.id()).unwrap().clone();
    ...
    assert!((t2.position.y - 1.5).abs() < 0.01 + cc2.skin_width, "Yere tam oturmalı: {}", t2.position.y);
}
...
fn test_character_step_climbing() {
    ...
    assert!(t.position.y > 1.6, "Basamağa tırmanamadı, Y={}", t.position.y);
}
...
fn test_character_slope_sliding() {
    ...
    let cc_out = world.borrow::<CharacterController>().unwrap().get(char_entity.id()).unwrap().clone();
    let t_out = world.borrow::<Transform>().unwrap().get(char_entity.id()).unwrap().clone();
    
    // Eğim aşıldığı için karakter dengesizleşmeli (is_grounded false olmalı)
    assert!(!cc_out.is_grounded, "Eğimi aştığı için yere bağlı sayılamaz!");
    // Yerçekimi ve slope sliding yüzünden daha da sağa/aşağı kaymış olmalı (X > 9.959, Y < 5.75)
    assert!(t_out.position.y < 5.7, "Rampadan aşağı doğru kayamadı! Y={}", t_out.position.y);
}
