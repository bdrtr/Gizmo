...
pub fn ui_console(ui: &mut egui::Ui, state: &mut EditorState) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.label(format!("({} Toplam)", state.console.count_info + state.console.count_warn + state.console.count_error));
        ...
        ui.checkbox(&mut state.console.show_info, format!("ℹ Info ({})", state.console.count_info));
        if prev_info != state.console.show_info { filter_changed = true; }

        let prev_warn = state.console.show_warn;
        ui.checkbox(&mut state.console.show_warn, format!("⚠️ Warn ({})", state.console.count_warn));
        if prev_warn != state.console.show_warn { filter_changed = true; }

        let prev_error = state.console.show_error;
        ui.checkbox(&mut state.console.show_error, format!("🔴 Error ({})", state.console.count_error));
    ...
    egui::ScrollArea::vertical()
        ...
        .show_rows(ui, 18.0, state.console.cached_logs.len(), |ui, row_range| {
            for i in row_range {
                ...
                let label = egui::Label::new(egui::RichText::new(&text).color(color)).wrap(true).sense(egui::Sense::click());
