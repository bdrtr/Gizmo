...
pub fn ui_hierarchy(ui: &mut egui::Ui, world: &World, state: &mut EditorState) {
    ...
    bg_response.context_menu(|ui| {
        ...
        if let Some(dragged) =
            ui.memory(|mem| mem.data.get_temp::<u32>(egui::Id::new("dragged_ent")))
        {
            if ui.input(|i| i.pointer.any_released()) {
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        for entity in world.iter_alive_entities() {
            let eid = entity.id();
            let has_parent = parents.as_ref().map(|p| p.contains(eid)).unwrap_or(false);
        ...
        egui::collapsing_header::CollapsingState::load_with_default_open(ui.ctx(), id, true)
            .show_header(ui, |ui| {
                ...
                if drag_response.hovered() {
                    if let Some(dragged) =
                        ui.memory(|m| m.data.get_temp::<u32>(egui::Id::new("dragged_ent")))
                    {
                ...
                response.context_menu(|ui| {
                    ...
                    if ui.button("📑 Çoğalt (Duplicate)").clicked() {
                        state.duplicate_requests.push(entity_id);
                        ui.close_menu();
                    }
