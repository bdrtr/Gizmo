...
and fast enough for ordinary unit tests. How to write one, what `output.shapes` can be asked, and
the two traps in building the state: the **`headless-editor-tests` skill**.

### Shader hot-reload (works, and is not discoverable)

Every renderer shader tries a **disk override** in `demo/assets/shaders/` before falling back to
its `include_str!` copy, and the studio rebuilds pipelines on any `.wgsl` change while it runs.
The mechanism and the workflow: the **`shader-hot-reload` skill**.
