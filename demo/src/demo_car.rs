...
pub fn setup_car_scene(
    ...
    let tbind_asphalt = gizmo::renderer::asset::AssetManager::new()
    ...
    world.add_component(
        ground_entity,
        Transform::new(Vec3::new(0.0, -1.0, 0.0)).with_scale(Vec3::new(500.0, 1.0, 500.0)),
    );
    world.add_component(
        ground_entity,
        Material::new(tbind_asphalt.clone()).with_pbr(Vec4::new(0.2, 0.2, 0.2, 1.0), 0.9, 0.1),
    );
    world.add_component(
        ground_entity,
    ...
    world.add_component(
        ramp1,
        Material::new(tbind_asphalt.clone()).with_unlit(Vec4::new(0.8, 0.4, 0.1, 1.0)),
    );
    world.add_component(ramp1, gizmo::renderer::components::MeshRenderer::new());
    let mut ramp_rb = RigidBody::new_static();
    ramp_rb.friction = 1.0;
    ...
    world.add_component(
        ice_field,
        Transform::new(Vec3::new(0.0, -0.9, 80.0)).with_scale(Vec3::new(40.0, 0.1, 80.0)),
    );
    world.add_component(
        ice_field,
        Material::new(tbind_asphalt.clone()).with_unlit(Vec4::new(0.6, 0.8, 1.0, 0.8)),
    );
    world.add_component(ice_field, gizmo::renderer::components::MeshRenderer::new());
    let mut ice_rb = RigidBody::new_static();
    ice_rb.friction = 0.05; // ÇOK KAYGAN
    ...
    for i in 0..5 {
        ...
        world.add_component(
            bump,
            Transform::new(Vec3::new(0.0, -0.7, 140.0 + (i as f32 * 8.0)))
                .with_scale(Vec3::new(20.0, 0.6, 2.0)),
        );
        world.add_component(
            bump,
            Material::new(tbind_asphalt.clone()).with_unlit(Vec4::new(0.9, 0.9, 0.2, 1.0)),
        );
        world.add_component(bump, gizmo::renderer::components::MeshRenderer::new());
        let mut bump_rb = RigidBody::new_static();
        bump_rb.friction = 0.8;
    ...
    world.add_component(
        sun,
        gizmo::renderer::components::DirectionalLight::new(Vec3::new(1.0, 1.0, 0.9), 1.5, true),
    );
    ...
    world.add_component(
        skybox,
        Material::new(tbind_asphalt.clone()).with_unlit(Vec4::new(0.4, 0.6, 0.9, 1.0)),
    );
    ...
    world.add_component(
        car,
        Material::new(tbind_asphalt.clone()).with_unlit(Vec4::new(0.8, 0.1, 0.1, 1.0)),
    );
