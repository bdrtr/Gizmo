use crate::{DragAxis, GameState, GizmoMode};
        ...
        (world.borrow::<Collider>(), world.borrow::<Transform>())
    {
        for i in 0..colliders.dense.len() {
            ...
            if let Some(t) = transforms.get(id) {
                if let gizmo::physics::ColliderShape::Aabb(aabb) = &colliders.dense[i].data.shape {
...
fn gizmo_axis_dir(hit: u32, state: &GameState) -> Vec3 {
    if hit == state.gizmo_x {
        Vec3::new(1.0, 0.0, 0.0)
    } else if hit == state.gizmo_y {
...
pub fn sync_gizmos(world: &mut World, state: &GameState) {
    let target_pos = if let Some(selected) = state.inspector_selected_entity {
        world
            .borrow::<Transform>()
            .and_then(|t| t.get(selected).map(|t| t.position))
    } else {
        None
    };
        ...
        if let Some(pos) = target_pos {
            ...
            for id in [state.gizmo_x, state.gizmo_y, state.gizmo_z] {
                if let Some(t) = trans.get_mut(id) {
                    t.position = Vec3::new(0.0, -1000.0, 0.0);
                }
            }
        }
    }
...
pub fn build_ray(
    ...
    if let (Some(cameras), Some(transforms)) = (
        ...
        if let (Some(cam), Some(cam_t)) = (cameras.get(player_id), transforms.get(player_id)) {
            ...
            let world_near = gizmo::math::Vec3::new(
                near_pt.x / near_pt.w,
                near_pt.y / near_pt.w,
                near_pt.z / near_pt.w,
            );
            let world_far = gizmo::math::Vec3::new(
                far_pt.x / far_pt.w,
                far_pt.y / far_pt.w,
                far_pt.z / far_pt.w,
            );
            return Some(gizmo::math::Ray::new(
                world_near,
