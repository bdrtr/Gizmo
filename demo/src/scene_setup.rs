...
pub fn spawn_gltf_hierarchy(
    ...
    for node in nodes {
        ...
        for (prim_i, (mesh, mat_opt)) in node.primitives.iter().enumerate() {
            ...
            world.add_component(
                prim_entity,
                EntityName(format!("{}_Primitive_{}", entity_name, prim_i)),
            );
        ...
        if !node.children.is_empty() {
            let child_entities =
                spawn_gltf_hierarchy(world, &node.children, Some(id), default_material.clone());
...
pub fn spawn_gltf_map_hierarchy(
    ...
    for node in nodes {
        ...
        let entity_name = node
            .name
            .clone()
            .unwrap_or_else(|| "GLTF_MapNode".to_string());
        ...
        for (prim_i, (mesh, mat_opt)) in node.primitives.iter().enumerate() {
            ...
            world.add_component(
                prim_entity,
                Transform::new(global_pos)
                    .with_rotation(global_rot)
                    .with_scale(global_scale),
            );
            world.add_component(
                prim_entity,
                EntityName(format!("{}_Primitive_{}", entity_name, prim_i)),
        ...
        if !node.children.is_empty() {
            let child_entities = spawn_gltf_map_hierarchy(
                world,
                &node.children,
                global_pos,
                global_rot,
...
pub fn setup_headless_scene(world: &mut World) -> crate::state::GameState {
    ...
    world.add_component(
        player,
        gizmo::physics::character::CharacterController::new(0.4, 0.8),
    );
...
pub fn spawn_gltf_asset(
    ...
    if !asset.skeletons.is_empty() {
        ...
        world.add_component(
            root_ent,
            gizmo::renderer::components::Skeleton {
...
pub fn setup_empty_scene(
    _world: &mut World,
    _renderer: &gizmo::renderer::renderer::Renderer,
) -> GameState {
...
pub fn setup_default_scene(
    ...
    let _sphere_mesh =
        gizmo::renderer::asset::AssetManager::create_sphere(&renderer.device, 1.0, 16, 16);
    ...
    world.add_component(
        ground_entity,
        Transform::new(Vec3::new(0.0, -1.0, 0.0)).with_scale(Vec3::new(200.0, 1.0, 200.0)),
    );

    let mut ground_mat =
        Material::new(tbind.clone()).with_pbr(Vec4::new(0.8, 0.7, 0.4, 1.0), 0.9, 0.1);
    ...
    world.add_component(
        ground_entity,
        gizmo::physics::shape::Collider::new_aabb(100.0, 0.05, 100.0),
    );

    // --- GÜNEŞ (Karanlık Ambiyans İçin Işık Şiddeti Çok Düşüldü) ---
    let sun = world.spawn();
    let sun_transform =
        Transform::new(Vec3::new(0.0, 50.0, 50.0)).with_rotation(Quat::from_axis_angle(
    ...
    world.add_component(
        sun,
        gizmo::renderer::components::DirectionalLight::new(Vec3::new(1.0, 0.95, 0.85), 2.0, true),
    ); // Güçlü Güneş Işığı
    ...
    world.add_component(
        x_gizmo,
        Transform::new(Vec3::new(0.0, -1000.0, 0.0)).with_scale(Vec3::new(1.5, 0.08, 0.08)),
    );
    world.add_component(
        x_gizmo,
        gizmo::renderer::asset::AssetManager::create_sphere(&renderer.device, 1.0, 6, 6),
    );
    world.add_component(
        x_gizmo,
        Material::new(tbind.clone()).with_unlit(Vec4::new(1.0, 0.0, 0.0, 1.0)),
    );
    ...
    world.add_component(
        y_gizmo,
        Transform::new(Vec3::new(0.0, -1000.0, 0.0)).with_scale(Vec3::new(0.08, 1.5, 0.08)),
    );
    world.add_component(
        y_gizmo,
        gizmo::renderer::asset::AssetManager::create_sphere(&renderer.device, 1.0, 6, 6),
    ...
    world.add_component(
        y_gizmo,
        Material::new(tbind.clone()).with_unlit(Vec4::new(0.0, 1.0, 0.0, 1.0)),
    );
    ...
    world.add_component(
        z_gizmo,
        Transform::new(Vec3::new(0.0, -1000.0, 0.0)).with_scale(Vec3::new(0.08, 0.08, 1.5)),
    );
    world.add_component(
        z_gizmo,
        gizmo::renderer::asset::AssetManager::create_sphere(&renderer.device, 1.0, 6, 6),
    ...
    world.add_component(
        z_gizmo,
        Material::new(tbind.clone()).with_unlit(Vec4::new(0.0, 0.0, 1.0, 1.0)),
    );
    ...
    let base_sphere_mesh =
        gizmo::renderer::asset::AssetManager::create_sphere(&renderer.device, 1.0, 8, 8); // 8x8
    ...
    if let Ok(cesium_man_asset) = asset_manager.load_gltf_scene(
        ...
        let man_root_transform = Transform::new(Vec3::new(0.0, 1.0, -10.0)).with_rotation(
    ...
    */
    ...
    for _ in 0..20 {
        ...
        if x.abs() < 5.0 && z.abs() < 5.0 {
            continue;
        } // Player başlangıcından uzağa koy
        ...
        world.add_component(
            wall,
            Transform::new(pos).with_scale(Vec3::new(1.0, 2.0, 1.0)),
        );
        world.add_component(
            wall,
            gizmo::renderer::asset::AssetManager::create_cube(&renderer.device),
        ...
        for ix in -pad_x..=pad_x {
            for iz in -pad_z..=pad_z {
                nav_grid.add_obstacle_world(
                    pos + Vec3::new(ix as f32 * cell_size, 0.0, iz as f32 * cell_size),
                );
            }
        }
    }
    ...
    world.add_component(
        npc,
        Transform::new(Vec3::new(20.0, 1.5, 20.0)).with_scale(Vec3::new(1.0, 2.0, 1.0)),
    );
    world.add_component(
        npc,
        gizmo::renderer::asset::AssetManager::create_cube(&renderer.device),
    ...
    world.add_component(
        npc,
        Material::new(tbind.clone()).with_pbr(Vec4::new(1.0, 0.0, 0.0, 1.0), 0.5, 0.5),
    ); // Kırmızı
    ...
    world.insert_resource(crate::state::DominoAppState {
        active_ball_id: None,
    });
    world.insert_resource(crate::state::PachinkoSpawnerState {
        timer: 0.0,
        count: 0,
    });
