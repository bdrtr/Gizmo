use gizmo::physics::{Collider, RaceAI, RigidBody, VehicleController, Wheel};
...
fn spawn_vehicle(
    ...
    world.add_component(entity, Transform::new(position).with_rotation(rotation));
    ...
    if let Some(path) = model_path {
        ...
        if let Ok(asset) = asset_manager.load_gltf_scene(
            ...
            let def_mat = gizmo::prelude::Material::new(tbind.clone()).with_pbr(
                Vec4::new(color[0], color[1], color[2], color[3]),
                0.5,
                0.1,
            );
            let _children = crate::scene_setup::spawn_gltf_hierarchy(
                world,
                &asset.roots,
                Some(entity.id()),
                def_mat,
            ...
            world.add_component(
                entity,
                gizmo::prelude::Material::new(tbind.clone()).with_pbr(
                    Vec4::new(color[0], color[1], color[2], color[3]),
                    0.5,
                    0.1,
                ),
...
pub fn setup_race_scene(
    ...
    if let Some(ref track_path) = config.track_model {
        if let Ok(asset) = asset_manager.load_gltf_scene(
            ...
            let def_mat = gizmo::prelude::Material::new(base_tbind.clone()).with_pbr(
            ...
            eprintln!(
                "[Race] Pist modeli yüklenemedi: {}. Prosedürel pist kullanılıyor.",
                track_path
            );
    ...
    world.add_component(
        ground,
        Transform::new(Vec3::new(0.0, config.ground_y, 0.0))
            .with_scale(Vec3::new(200.0, 1.0, 200.0)),
    );
    world.add_component(ground, RigidBody::new_static());
    world.add_component(ground, Collider::new_aabb(200.0, 0.5, 200.0));
    world.add_component(
        ground,
        gizmo::prelude::Material::new(base_tbind.clone()).with_pbr(
            Vec4::new(0.2, 0.5, 0.15, 1.0),
            0.8,
            0.1,
        ),
    );
    ...
    world.add_component(
        sun,
        gizmo::renderer::components::DirectionalLight::new(Vec3::new(1.0, 0.95, 0.85), 2.5, true),
    );
    ...
    world.add_component(
        camera_entity,
        Transform::new(config.grid_positions[0] + Vec3::new(0.0, 5.0, -10.0)),
    );
    world.add_component(
        camera_entity,
        Camera {
    ...
    for i in 0..ai_count {
        ...
        world.add_component(
            ai_entity,
            RaceAI::new(config.waypoints.clone(), config.ai_speeds[i], 8.0),
        );
...
fn build_fallback_track(
    ...
    let blocks: Vec<(Vec3, Vec3, [f32; 4])> = vec![
        ...
        (
            ...
            Vec3::new(road_half_w, 0.1, straight_len),
            road_color,
        ),
        // Düz yol 2
        (
            Vec3::new(curve_offset, 0.01, 0.0),
            Vec3::new(road_half_w, 0.1, straight_len),
            road_color,
        ),
        // Viraj 1 (kuzey)
        (
            Vec3::new(curve_offset / 2.0, 0.01, straight_len + road_half_w),
            Vec3::new(curve_offset / 2.0 + road_half_w, 0.1, road_half_w),
            road_color,
        ),
        // Viraj 2 (güney)
        (
        ...
        // Sol duvar yol 1
        (
            Vec3::new(-road_half_w - wall_thick, wall_h / 2.0, 0.0),
            Vec3::new(wall_thick, wall_h / 2.0, straight_len + road_half_w),
            wall_color,
        ),
        // Sağ duvar yol 2
        (
            Vec3::new(curve_offset + road_half_w + wall_thick, wall_h / 2.0, 0.0),
            Vec3::new(wall_thick, wall_h / 2.0, straight_len + road_half_w),
            wall_color,
        ),
        // Kuzey dış duvar
        (
            Vec3::new(
                curve_offset / 2.0,
                wall_h / 2.0,
                straight_len + road_half_w * 2.0 + wall_thick,
            ),
            Vec3::new(
                curve_offset / 2.0 + road_half_w + wall_thick,
                wall_h / 2.0,
                wall_thick,
            ),
            wall_color,
    ...
    match race.phase {
        RacePhase::Countdown => {
            ...
            if let Some(mut vehicles) = world.borrow_mut::<VehicleController>() {
                for e in vehicles.dense.iter().map(|e| e.entity).collect::<Vec<_>>() {
        ...
        RacePhase::Racing => {
                                    ...
                                    if dist < 8.0 {
                                        ...
                                        if race.player_last_checkpoint >= race.checkpoint_count {
                                            ...
                                            if race.player_laps >= race.total_laps {
                                                race.finish_order
                                                    .push((race.player_entity, race.race_timer));
                                            }
                                        }
                                    }
                                }
            ...
            if race.finish_order.len() >= all_count
                || race
                    .finish_order
                    .iter()
                    .any(|(id, _)| *id == race.player_entity)
            {
