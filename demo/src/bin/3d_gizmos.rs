//! # Anlık-mod hata ayıklama çizimi
//!
...
//!
//! **Motorun çizim yüzeyi dar, ve demo bunu saklamıyor.** Izgara, küre, dikdörtgen, artı işareti,
//! ok ya da hazır bir 3B ilkel için tek çağrılık kısayol **yok**; `Gizmos` yalnız dört şey
//! biliyor: [`Gizmos::draw_line`], [`Gizmos::draw_box`], [`Gizmos::draw_aabb`] ve
//! [`Gizmos::draw_frustum`]. Dolayısıyla bu demodaki ızgara, küre, ok ve eğri **çizgilerden
//! kuruluyor** — ve o yardımcılar dosyanın altında duruyor, çünkü asıl öğretici olan da bu: elde
//! olan ilkel `draw_line`, geri kalanı üç satırlık geometri.
//!
//! Kalıcı `Gizmo` varlığı (30 000 çizgilik statik bir küre gibi, bir kez yazılıp öylece duran
//! çizgi kümesi) de motorda yok: burada her şey her frame yeniden yazılıyor, ki `Gizmos::clear`
//! + yeniden çizim tam olarak bu demoda görülüyor.
//!
//! Sahnenin geri kalanı şöyle: 5×5 zemin (`srgb(0.3, 0.5, 0.3)`), orijinde
//! 1×1×1 küp (`srgb(0.8, 0.7, 0.6)`), `(4, 8, 4)`'te nokta ışığı, `(0, 1.5, 6)`'dan orijine bakan
//! kamera.
//!
//!
//! **Bir motor tuzağı, ve ilk bakışta görünmez:** `add_system` sistemi **sabit
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - 3D Gizmos", 1280, 720)
...
/// Her frame tamponu sıfırla, her şeyi yeniden çiz.
fn draw_example_collection(
    ...
    // (10, 10, 10)'da mor bir küre ve onu taşıyan ikinci ızgara.
...
// ── Motorda hazır olmayan, burada çizgiden kurulan ilkel şekiller ─────────────────────────
