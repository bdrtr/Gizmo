//! # Bevy'nin `3d/fog` örneğinin Gizmo Engine portu
//!
//! Uzaklıkla kapanan görüş: uzaktaki nesnelerin sis rengine karışması. Bevy'de bu, kameraya
//! takılan bir `DistanceFog` bileşeni — rengi, düşüş eğrisi (`Linear` / `Exponential` /
//! `ExponentialSquared` / `Atmospheric`) ve mesafeleri çalışma anında yazılabiliyor.
//!
...
//!
//! Yani Bevy'nin bileşeninde tek tek yazılan her şey burada **seçilebilir, ayarlanamaz**.
...
//! seçilemiyor; Bevy'nin dört düşüş kipinin karşılığı yok.
//!
...
//! bütün havasını değiştirmek demek; Bevy'de sis bileşeni ötekilerden bağımsızdır.
//!
...
//! kalıp sahneyi ufkun üstünde bıçak gibi kesiyor. Bevy örneği bunu `ClearColor` ile çözüyor.
//! Buradaki karşılık ters çevrilmiş dev bir küp: ışıksız bir malzemeyle, rengi her kare seçili
//! ayardan alınıyor. (Motorun asıl cevabı `MaterialType::Skybox` ama o bir küp haritası ister,
//! depoda yok.)
//!
//! ## Motorun ikinci sisi
//!
...
//! sisinin yerine geçmiyor — bu yüzden bu portun konusu değil.
//!
//! ## Sahne
//!
//! Bevy tek bir piramit kuruyor; burada uzaklığa doğru dizilmiş **beş piramit** var, çünkü sisin
//! ölçüsü mesafeyle değişimi ve onu tek bir nesne göstermiyor. En yakını 8 m, en uzağı 72 m'de.
//!
//! ## Kontroller
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Fog", 1280, 720)
        .with_simple_scene(|scene, state| {
            ...
            for (i, z) in DEPTHS.into_iter().enumerate() {
                // Kademeli piramit: Bevy'nin yaptığı gibi, derinliği okunaklı kılan bir yapı.
            ...
            // gibi kesiliyor. Bevy örneği bunu `ClearColor` ile çözüyor; motorun karşılığı ya bir
            // skybox (küp haritası ister, depoda yok) ya da buradaki gibi ters çevrilmiş dev bir
            // küp. Rengi her kare seçili ayardan alınıyor.
            scene.world.spawn_bundle((
        ...
        .set_ui(|world, _state, ctx| {
            ...
            gizmo::egui::Area::new("fog".into())
                ...
                .show(ctx, |ui| {
                    ...
                    ui.label("Bevy'de renk, yoğunluk ve düşüş kipi bileşende ayarlanır.");
                    ui.separator();
...
fn fog_input(mut fog: ResMut<FogState>, input: Res<Input>, time: Res<Time>) {
    ...
    // Bevy'de sisin sürekli sayıları (yoğunluk, mesafe, renk kanalları) tuşla değişiyor; burada
