...
//!
//! Demo A tuşunu izliyor ve üç kenarı hem günlüğe yazıyor hem ekranda
...
//!
//! ## Mantıksal tuş sorgusu motorda yok, ve karşılığı da yok
//!
//! **Mantıksal** tuşu — `'?'` karakteri, klavye düzeninden bağımsız — soracak bir yol **yok**.
//! Gizmo'nun `Input`'u yalnız **fiziksel** tuş kodlarını tutuyor
...
//! yalnız "hangi konumdaki tuşa basıldı" sorusunu cevaplar. Bu demo o eksiği taklitle kapatmıyor;
//! motorun boşluğu olarak duruyor.
//!
//! **Zamanlama:** girdi okuyan sistem `add_update_system` ile kaydedilir — kare çizelgesi
//! kenarların yakalanıp temizlendiği yerdir (bkz. `fixed_timestep`).
//!
//! ## Kontroller
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Keyboard Input", 1280, 720)
        ...
        .set_ui(|world, _state, ctx| {
            ...
            gizmo::egui::Area::new("keyboard".into())
                ...
                .show(ctx, |ui| {
                    ...
                    ui.monospace(line);
                    ui.separator();
                    ui.label("mantıksal tuş ('?') sorgusu motorda yok:");
...
/// Aynı tuşun üç kenarını da ayrı ayrı soran sistem.
