//! # Bevy'nin altı ışık örneği: Gizmo'da karşılığı olmayanlar
//!
//! Bu demo altı Bevy örneğini birden karşılıyor — `3d/rect_light`, `3d/spherical_area_lights`,
//! `3d/light_textures`, `3d/contact_shadows`, `3d/pcss`, `3d/pccm` — ve altısının da cevabı aynı:
...
//! yok.
//!
//! | Bevy örneği | ne gerekiyor | Gizmo |
//! |-------------|--------------|-------|
//! | `rect_light` | dikdörtgen alan ışığı (LTC) | **yok** — `ltc`/`area_light` diye bir şey aranmıyor bile |
//! | `spherical_area_lights` | küresel alan ışığı, yarıçapı gölgeyi yumuşatır | **yok** — `PointLight::radius` **menzil**, kaynak boyutu değil |
//! | `light_textures` | ışığa doku (cookie/gobo) takmak | **yok** |
//! | `contact_shadows` | ekran uzayında temas gölgesi | **yok** |
//! | `pcss` | mesafeye göre yumuşayan gölge (blocker araması) | **yok** — PCF **sabit ~1 teksel** |
//! | `pccm` | paralaks düzeltmeli küp haritası yansıması | **yok** |
//!
//! ## `PointLight::radius` bir tuzak
//!
//! Bevy'de bir nokta ışığının `radius`'u **kaynağın fiziksel boyutu**: büyüttükçe gölge yumuşar,
//! ışık alan ışığına dönüşür. Gizmo'da aynı isim **menzil** demek — belgesi açık: *"düşüşün sıfıra
//! ulaştığı mesafe, ve aynı zamanda ayıklama sınırı."*
//!
//! Yani Bevy'den gelen biri `radius`'u büyütünce yumuşak gölge bekler, **daha geniş ve daha zayıf
//! bir aydınlatma** bulur. Demo bunu ölçüyor.
//!
...
//! | 20 | **143,16** |
//!
//! Yarıçap on kat büyüyünce sahne **aydınlanıyor** — daha çok yüzey menzile giriyor. Bevy'de
//! aynı hareket ışığı bir alan ışığına çevirir ve gölgeleri yumuşatır; burada yalnız erişim
//! genişliyor. İsim aynı, anlam farklı.
//!
...
/// Nokta ışığının yarıçapı — Bevy'de kaynak boyutu, burada menzil.
fn light_radius() -> f32 {
...
fn main() {
    ...
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Area Lights", 1280, 720)
