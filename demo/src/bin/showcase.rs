...
fn main() {
    App::<ShowcaseDemo>::new("Gizmo — AAA Showcase (IBL, SSR, God Rays)", 1600, 900)
        .set_setup(|world, renderer| {
            ...
            world.add_component(
                l1,
                gizmo::renderer::components::PointLight::new(Vec3::new(1.0, 0.0, 0.0), 50.0, 10.0),
            );
            ...
            world.add_component(
                l2,
                gizmo::renderer::components::PointLight::new(Vec3::new(0.0, 1.0, 0.0), 50.0, 10.0),
            );
            ...
            world.add_component(
                l3,
                gizmo::renderer::components::PointLight::new(Vec3::new(0.0, 0.5, 1.0), 50.0, 10.0),
            );
            ...
            if let Some(_decal_state) = renderer.decal.as_ref() {
                ...
                world.add_component(
                    decal_entity,
                    Transform::new(Vec3::new(0.0, 0.0, -10.0)).with_scale(Vec3::new(8.0, 8.0, 8.0)), // 8x8x8 volume
                );
                world.add_component(
                    decal_entity,
        ...
        .set_update(|world, state, dt, input| {
            ...
            // Camera: move via Transform (the renderer falls back to the camera's own
            // Transform when it has no GlobalTransform) and mirror yaw/pitch into the
            // Camera component (the renderer takes the view *direction* from there).
            {
                let cam_id = state.cam_id;
                if let Some(mut t) = world.borrow_mut::<Transform>().get_mut(cam_id) {
                    t.position = state.cam_pos;
                    t.rotation = pitch_yaw_quat(state.cam_pitch, state.cam_yaw);
                    t.update_local_matrix();
                }
                if let Some(mut cam) = world.borrow_mut::<Camera>().get_mut(cam_id) {
            ...
            if input.is_mouse_button_pressed(0) && state.time - state.last_shot_time > 0.1 {
                ...
                world.add_component(
                    bullet,
                    Transform::new(state.cam_pos).with_scale(Vec3::new(0.5, 0.5, 0.5)),
                );
            ...
            state.request_force_pull = input.is_key_pressed(KeyCode::KeyF as u32);
        ...
        .set_ui(|_world, state, ctx| {
            gizmo::egui::Window::new("Gizmo AAA Engine")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.time_of_day, 0.0..=24.0)
                            .text("Günün Saati"),
                    );
                    ui.separator();
                    ui.label(gizmo::egui::RichText::new("Zaman Kontrolü (Matrix Etkisi)").strong());
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.time_scale, 0.0..=2.0)
                            .text("Zaman Ölçeği (Ağır Çekim)"),
                    );
                    ui.separator();
                    ui.label(gizmo::egui::RichText::new("Sinematik Post-Process").strong());
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.bloom_intensity, 0.0..=2.0)
                            .text("Bloom Yoğunluğu"),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.exposure, 0.1..=5.0)
                            .text("Pozlama (Exposure)"),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.chromatic_aberration, 0.0..=5.0)
                            .text("Renk Sapması (Chromatic Ab.)"),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.vignette_intensity, 0.0..=2.0)
                            .text("Vignette"),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.film_grain_intensity, 0.0..=1.0)
                            .text("Film Greni"),
                    );
                    ui.separator();
                    ui.label(gizmo::egui::RichText::new("Alan Derinliği (DoF)").strong());
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.dof_focus_dist, 0.1..=100.0)
                            .text("Odak Uzaklığı"),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.dof_focus_range, 0.1..=50.0)
                            .text("Odak Aralığı"),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.dof_blur_size, 0.0..=10.0)
                            .text("Bulanıklık Miktarı"),
                    );
                    ui.separator();
                    ui.label(
                        gizmo::egui::RichText::new("Parçacık Efekti (GPU Particles)").strong(),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.particle_spawn_rate, 0..=1000)
                            .text("Üretim Hızı (Kare Başına)"),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.particle_velocity_y, 0.0..=50.0)
                            .text("Fışkırma Gücü (Y)"),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.particle_color_r, 0.0..=1.0)
                            .text("Renk (Kırmızı)"),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.particle_color_g, 0.0..=1.0)
                            .text("Renk (Yeşil)"),
                    );
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.particle_color_b, 0.0..=1.0)
                            .text("Renk (Mavi)"),
                    );
                    ui.separator();
                    ui.label(gizmo::egui::RichText::new("Fizik Motoru (GPU Physics)").strong());
                    ui.add(
                        gizmo::egui::Slider::new(&mut state.gravity_y, -20.0..=20.0)
                            .text("Yerçekimi (Y)"),
                    );
                });
            ...
            painter.circle_filled(
                center,
                2.0,
                gizmo::egui::Color32::from_rgba_unmultiplied(255, 255, 255, 220),
            );
        ...
        .set_render(|world, state, encoder, view, renderer, _light_time| {
            ...
            if let Some(physics) = &renderer.gpu_physics {
                physics.update_params(
                    &renderer.queue,
                    0.016 * state.time_scale,
                    [0.0, state.gravity_y, 0.0],
                );
                ...
                if state.request_force_pull {
                    ...
                    if let Some(gpu_data) = physics.poll_readback_data(&renderer.device) {
                        for (idx, mut box_data) in gpu_data.into_iter().enumerate() {
                            ...
                            let is_in_front =
                                dist_to_cam > 0.1 && (to_obj_dir.normalize().dot(fwd) > 0.7);
                            ...
                            if is_in_front && dist_to_cam < 50.0 {
                                ...
                                let force_mag = if dist_to_target > 3.0 {
                                    pull_force
                                } else {
                                    pull_force * 0.1
                                };
                                ...
                                let lift = if box_data.velocity[1].abs() < 1.0 {
                                    30.0
                                } else {
                                    0.0
                                };
                                ...
                                if dist_to_target < 4.0 {
                                    ...
                                    box_data.velocity[1] =
                                        box_data.velocity[1] * 0.8 + (9.81 * 0.016); // Yerçekimine karşı gelip havada tutar
            ...
            if let Some(particles) = &renderer.gpu_particles {
                ...
                for _ in 0..state.particle_spawn_rate {
                    ...
                    new_particles.push(gizmo::renderer::gpu_particles::GpuParticle {
                        ...
                        max_life: life,
                        color: [
                            state.particle_color_r,
                            state.particle_color_g,
                            state.particle_color_b,
                            1.0,
                        ], // Parlak renkli
                        size_start: 0.5,
                        size_end: 0.01,
