...
//! açılabilsin. İşin püf noktası işaret bileşeni değil, **örtük filtre** — motor *her* sorguya
//! sessizce `Without<Disabled>` ekleseydi, devre dışı varlık hiçbir sıradan sorguda görünmezdi.
//!
//! ## Motorda yok, ve eksik olan asıl parça filtre
//!
//! | yetenek | Gizmo |
//! |---------|-------|
//! | devre dışı işaret bileşeni | **yok** (elle bir işaret bileşeni yazılır) |
//! | her sorguya örtük filtre | **yok** |
//! | örtük filtreyi tek bir sorgu için devre dışı bırakma | **yok** |
//! | işareti varlığa takmak | `add_component` ile aynı |
//!
//! İşaret bileşenini yazmak kolay. **Örtük filtre yazılamıyor** — ve asıl iş orada. Örtük filtre
//! olsaydı işaret takılan varlık her sorgudan kendiliğinden düşerdi; Gizmo'da her sorguya
...
//!
//! Örtük filtre olan bir motorda bu hata **yazılamaz**: filtre motorun içinden her sorguya eklenir,
//! ve devre dışı varlığı görmek isteyen istisnayı açıkça yazmak zorunda kalır. Yani varsayılan
//! güvenli, istisna açık olurdu; Gizmo'da varsayılan sızdırıyor, güvenlik açık.
//!
//! Eksik olan parça bileşen değil — **örtük filtre**. Ve o filtre kullanıcı kodunda yazılamıyor,
//! çünkü sorgu türleri mühürlü (bkz. `ecs_extension_points`).
//!
//! ## Kontroller
...
/// Devre dışı işaretinin elle yazılmış hâli. Bileşen kolay; eksik olan **örtük filtre**.
#[derive(Clone, Copy)]
struct Disabled;
...
struct DisableReport {
    ...
    unfiltered: usize,
    /// Devre dışı olanların kaç kez yine de işlendiği — örtük filtre olsaydı bu sayı 0 olurdu.
    leaked_updates: u64,
}
            ...
            gizmo::egui::Area::new("dis".into())
                ...
                .show(ctx, |ui| {
                    gizmo::egui::Frame::popup(ui.style()).show(ui, |ui| {
                        ...
                        ui.separator();
                        ui.label("örtük filtre her sorguya Without<Disabled> ekleseydi");
                        ui.label("filtreyi unutmak MÜMKÜN DEĞİL olurdu.");
...
/// Filtreyi **unutan** sistem — örtük filtre olsaydı bu hata yazılamazdı.
fn count_unfiltered(
