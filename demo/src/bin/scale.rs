...
//!
//! **Bu demo, [`3d_rotation`]'ın tersini gösteriyor.** Orada elle yazılan
...
//! kayıt. İki demonun yan yana durması, "motor ne zaman yardım eder, ne zaman etmez"in
//! kendisidir.
//!
//! Sayısal notlar:
//!   * Küp 1×1×1 birim varsayılır — Gizmo'nun hazır küpü 2 birim olduğundan ölçek
//!     değerleri 0,5 ile çarpılarak uygulanır; sınırlar 1,0 ve 5,0 *birim*
//!     cinsinden tutulur.
...
/// `Scaling` bileşeni: hangi eksende, ne hızla, hangi sınırlar arasında.
#[derive(Clone, Copy)]
struct Scaling {
...
/// Gizmo'nun hazır küp mesh'i 2 birim; 1 birimlik küpe getiren çarpan.
const UNIT: f32 = 0.5;
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Scale", 1280, 720)
...
///
/// `floor`/`ceil` numarası: sınır aşıldığında ölçek tam sayıya
...
fn change_scale_direction(mut cubes: Query<(Mut<Transform>, Mut<Scaling>)>) {
    for (_entity, (mut transform, mut cube)) in cubes.iter_mut() {
        // Gizmo'nun küpü 2 birim olduğu için sınırlar *birim* ölçeğinde okunur.
        ...
        if size.min_element() < cube.min_element_size {
            ...
            // `.zxy()` swizzle'ı — motorun matematiği glam ama `Vec3Swizzles`
