//! # ECS genişletme noktaları
//!
//! Bu demo **dört** ayrı yeteneği birden ele alıyor, çünkü dördünün de cevabı aynı yerden
//! geliyor: kendi sistem parametreni yazmak, kendi sorgu operandını yazmak, sistemleri boru
//! hattına dizmek, bir sistemi çizelge dışından tek seferlik koşturmak.
//!
...
//! | yetenek | Gizmo |
//! |---------|-------|
//! | kendi sistem parametreni yazmak | `SystemParam: sealed::Sealed` — **uygulanamaz** |
//! | kendi sorgu operandını yazmak | `WorldQuery: sealed::SealedQuery`, `ReadOnlyQuery: sealed::SealedReadOnly`, `FetchComponent: sealed::SealedFetch` — **üçü de mühürlü** |
//! | bir sistemin çıktısını ötekine boru hattıyla vermek | `pipe` diye bir şey **yok** |
//! | bir sistemi çizelge dışından tek seferlik koşturmak | `run_system` **yok**; sistemler yalnız çizelgeden koşar |
//!
...
//! yazılmış bir `SystemParam` bu bilgiyi veremezdi. Ama sonucu şu: bu dört yeteneğin hiçbirinin
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - ECS Extension Points", 1280, 720)
            ...
            gizmo::egui::Area::new("ext".into())
                ...
                .show(ctx, |ui| {
                    gizmo::egui::Frame::popup(ui.style()).show(ui, |ui| {
                        ...
                        ui.label("dört yeteneğin cevabı aynı yerden geliyor");
                        ui.separator();
...
/// Boru hattının ikinci ucu — `pipe` olsaydı sağ tarafı, burada düz bir fonksiyon.
