//! # Nesne başına gölge denetimi
//!
//! Nesne başına gölge ayarı: bu gölge **düşürsün mü**, bu gölge **alsın mı**.
//!
//! ## Motorda yarısı var: düşürme tarafı denetlenebiliyor, alma tarafı değil
//!
//! | yetenek | Gizmo |
//! |---------|-------|
//! | gölge düşürmeyi kapatmak | [`ShadowCasting::Off`] |
//! | gölge düşürüp kendisi çizilmemek | **[`ShadowCasting::Only`]** — gölge düşürür ama **çizilmez** |
//! | gölge almayı kapatmak | **yok** — alma tarafı kapatılamıyor |
//!
//! Yani düşürme tarafı ifadeli: üçüncü bir hâl var, ve gerekçesi kodun kendisinde
//! yazılı — *"pahalı bir mesh için gölge düşüren düşük poligonlu bir vekil, ya da görünmeden ışığı
//! şekillendiren bir engel."*
//!
...
fn main() {
    let mode = mode();
    App::<SimpleSceneState>::new("Gizmo Engine - Shadow Caster/Receiver", 1280, 720)
            ...
            gizmo::egui::Area::new("shadow".into())
                ...
                .show(ctx, |ui| {
                    gizmo::egui::Frame::popup(ui.style()).show(ui, |ui| {
                        ...
                        ui.separator();
                        ui.label("Only: gölge düşürür ama ÇİZİLMEZ.");
                        ui.label("gölge ALMAYI kapatmak: motorda YOK.");
                        ui.separator();
