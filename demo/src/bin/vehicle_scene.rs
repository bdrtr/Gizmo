...
fn setup(world: &mut World, renderer: &Renderer) -> VehicleState {
    ...
    let mut ramp_trans =
        Transform::new(Vec3::new(0.0, 2.0, -30.0)).with_scale(Vec3::new(8.0, 0.5, 15.0));
    ...
    world.add_component(
        camera_ent,
        Camera::new(std::f32::consts::FRAC_PI_3, 0.1, 5000.0, 0.0, -0.3, true),
    );
...
fn update(world: &mut World, state: &mut VehicleState, dt: f32, input: &gizmo::core::input::Input) {
    ...
    // --- ARAÇ GİRDİLERİ ---
    let throttle = if input.is_key_pressed(gizmo::winit::keyboard::KeyCode::KeyI as u32) {
        1.0_f32
    } else {
        0.0
    };
    let reverse = input.is_key_pressed(gizmo::winit::keyboard::KeyCode::KeyK as u32);
    let brake = if input.is_key_pressed(gizmo::winit::keyboard::KeyCode::Space as u32) {
        1.0_f32
    } else {
        0.0
    };
    let steering = if input.is_key_pressed(gizmo::winit::keyboard::KeyCode::KeyJ as u32) {
        1.0_f32
    } else if input.is_key_pressed(gizmo::winit::keyboard::KeyCode::KeyL as u32) {
        -1.0
    } else {
        0.0
    };
