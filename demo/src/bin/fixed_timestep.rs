...
//! süresiyle (oyun mantığı, girdi, kamera). Ayrım kaydın kendisinde: `add_system` sabit
//! çizelgeye, `add_update_system` kare çizelgesine yazar.
//!
...
//!
//! Demo bu sayıları hem HUD'da gösteriyor hem de günlüğe yazıyor — ve üstüne **görünür** kılıyor:
//! iki küp aynı sinüsü aynı hızla tarıyor, biri sabit adımlarda (kesikli), diğeri her karede
//! (sürekli). Aynı hizada kalmaları, iki çizelgenin aynı gerçek zamanı harcadığının kanıtı.
//!
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Fixed Timestep", 1280, 720)
...
///
/// Küpü sabit adımlarla ilerletir — hareket bu yüzden **kesikli**, ve düşük FPS'te bile aynı
/// hızda: adım sayısı gerçek zamana göre ayarlanır.
fn fixed_update(
...
///
/// Aynı zamanda bir önceki karede kaç sabit adım koştuğunu da defterler — bu demonun asıl
/// ölçtüğü sayı o.
fn frame_update(
    ...
    // Her 120 karede bir aynı sayılar günlüğe düşer.
