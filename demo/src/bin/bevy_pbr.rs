//! # Bevy'nin `3d/pbr` örneğinin Gizmo Engine portu
//!
...
//!
//! ## Bu portun dürüst olması gereken yer: ortam haritası (IBL) yok
//!
//! Bevy'nin örneği kameraya bir `EnvironmentMapLight` takar — metalik satırların gördüğü ışığın
//! neredeyse tamamı o ortam haritasından gelir, çünkü **saf metal difüz yansıtmaz**: yalnız
//! çevresini yansıtır. Gizmo'da ortam küp haritası / yansıma sondası **yok** (motorun bilinen
//! boşluklarından biri). Sonuç şu: bu ızgarada `roughness` ekseni Bevy'dekiyle aynı okunur, ama
//! üst satırlara doğru gidildikçe küreler **kararır** — yansıtacak bir çevre olmadığı için.
//! Bu bir port hatası değil, motorun eksiğinin görüntüsü; demoyu "düzeltmek" için sahte bir
//! ortam ışığı eklemedim.
//!
//! Sayısal parite notları (Bevy 0.19 kaynağına karşı):
//!   * Küre yarıçapı 0,45; ızgara `x ∈ [−5, 5]`, `y ∈ [−2, 2]`; renk `#ffd891` → doğrusal
//!     `(1.000, 0.687, 0.283)`.
//!   * Kamera `(0, 0, 8)`'den orijine bakar ve **ortografiktir**. Bevy `scale: 0.01` +
//!     `ScalingMode::WindowSize` diyor, yani görüş yüksekliği = pencere yüksekliği × 0,01;
//!     720 piksellik pencerede **7,2 dünya birimi** — buraya o değer yazıldı.
...
/// Bevy'nin `scale: 0.01` × 720 piksellik pencere yüksekliği.
const VIEWPORT_HEIGHT: f32 = 7.2;

fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy PBR", 1280, 720)
