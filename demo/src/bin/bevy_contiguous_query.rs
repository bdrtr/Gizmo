//! # Bevy'nin `ecs/contiguous_query` örneğinin Gizmo Engine portu
//!
//! Bileşenleri satır satır değil, **bitişik dilimler** hâlinde gezmek — SIMD ve önbellek dostu
//! toplu işler için. Bevy'de bu `Query::iter_contiguous()`.
//!
//! ## Motorda var, ve adı `iter_chunks`
//!
//! | Bevy | Gizmo |
//! |------|-------|
//! | `query.iter_contiguous()` | [`Query::iter_chunks`] |
...
//!
//! İkincisi tasarım gereği ve doğru olan davranış, ama Bevy'den gelen biri için sürpriz:
//! `iter_contiguous` orada filtreyle de çalışıyor.
//!
//! ## Ölçüldü
//!
...
//!
//! ### Küçük bir fark: dilim varlık id'lerini de taşıyor
//!
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Contiguous Query", 1280, 720)
            ...
            gizmo::egui::Area::new("chunks".into())
                ...
                .show(ctx, |ui| {
                    gizmo::egui::Frame::popup(ui.style()).show(ui, |ui| {
                        ...
                        ui.label("Bevy: iter_contiguous · Gizmo: iter_chunks");
                        ui.separator();
                        ui.monospace(format!(
