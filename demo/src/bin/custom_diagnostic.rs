//! # Kendi ölçümlerini kaydetmek
//!
//! Kendi ölçümünü kaydetmek: bir sistem her karede bir sayı üretiyor, motor onu biriktiriyor,
//! ve sonuç istatistikleriyle birlikte okunabiliyor. Gizmo'da bunun yeri `gizmo-analysis`
//! crate'i.
//!
//! **Bu, o crate'i gösteren ilk demo** — bugüne kadar hiçbir demo ona dokunmuyordu, ve
//! `demo`'nun özellik listesinde bile yoktu (bu demo `analysis` özelliğini açıyor).
//!
//! ## Motorun verdiği: üç ayrı seri türü
//!
//! Gizmo'nun [`Analyzer`]'ı üç ayrı seri türü tanıyor — **sayaç** (`counter_add`), **gösterge**
//! (`gauge`) ve **örnek** (`sample`) — ve her biri için min/maks/ortalama/std/p50/p95/p99
//! hesaplıyor. Demo üçünü de kullanıyor:
//!
//!   * `demo.system_iterations` — sayaç: ölçüm sistemi kaç kez koştu.
...
//! `set_render` kapanışı.)
//!
//! Ölçümler iki yerden okunuyor: HUD (canlı istatistikler) ve **F3** — `Analyzer::report_text()`'i
//! günlüğe basar.
//!
//! ## Kontroller
...
/// Ölçümlerin adları — sabit tutuluyor, çünkü seriyi yazan ile okuyan aynı dizeyi kullanmalı.
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Custom Diagnostic", 1280, 720)
...
/// Ölçüm sistemi: her karede ölçümleri kaydeder.
///
...
fn measure(
    ...
    // Sayacın ölçtüğü şey tam olarak bu: sistemin kaç kez koştuğu.
