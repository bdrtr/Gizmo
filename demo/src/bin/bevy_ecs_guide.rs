//! # Bevy'nin `ecs/ecs_guide` örneğinin Gizmo Engine portu
//!
//! Bevy'nin ECS'e giriş turu: oyuncular (varlık + bileşen), oyun durumu ve kuralları (kaynak),
...
//!
//! **Bu, seri içinde penceresiz koşan ilk demo.** Bevy'nin örneği `ScheduleRunnerPlugin` ile
//! konsolda çalışıyor; motorun karşılığı `gizmo::app::headless::App` — GPU'suz, pencere açmayan,
//! her zaman derlenen ikinci çalışma zamanı. Çıktı ekranda değil, terminalde.
//!
//! ## Bevy'de olup Gizmo'da olmayan üç şey — üçü de burada elle yazıldı
//!
//! **1. `Local<T>` yok.** Bevy'nin `print_at_end_round(mut counter: Local<u32>)`'ı sisteme ait,
//! dünyada görünmeyen bir sayaç tutuyor. Gizmo'da böyle bir sistem parametresi yok; karşılığı
//! sıradan bir kaynak ([`RoundLog`]). Fark yalnız üslup değil: kaynak dünyada durur, yani başka
//! her sistem onu okuyabilir ve çakışma çizelgeye görünür.
//!
//! **2. `exclusive()` bir yetki değil, bariyer.** Bevy'nin dışlayıcı sistemi `&mut World` alır ve
//! dünyayı doğrudan değiştirir. Gizmo'nun [`SystemConfig::exclusive`]'i — kendi belgesinin
...
//! sistemin kendi partisinde yalnız koşması. Demodaki `exclusive_player_system` bu yüzden
//! Bevy'dekiyle aynı işi yapıyor ama `Commands` ile.
//!
//! **3. `AppExit` iletisi yok.** Bevy oyunu bitirince `AppExit` yazıyor ve döngü kapanıyor.
...
//!
//! ## Dördüncü tuzak, bu port sırasında bulundu: `set_runner` kurulumu yutuyor
//!
...
//!
//! ```text
//! cargo run --release -p demo --bin bevy_ecs_guide
//! ```
...
/// Bevy'nin `Local<u32>`'unun karşılığı: sisteme ait olamayan, o yüzden dünyada duran sayaç.
...
/// Bevy'nin `print_at_end_round`'u — `Local<u32>` yerine [`RoundLog`] kaynağıyla.
...
/// Bevy'nin dışlayıcı sisteminin karşılığı.
///
/// `.exclusive()` bu sistemi kendi partisinde yalnız bırakır ama `&mut World` **vermez** — o
/// yüzden iş yine `Commands` üzerinden yapılıyor. Bevy'deki sürümle aynı sonucu üretir, sadece
/// dünyaya erişme yolu farklıdır.
...
fn main() {
    let app = App::<()>::new("Gizmo Engine - Bevy ECS Guide", 0, 0)
        ...
        // Motorda `AppExit` yok: turları saymak ve durmak koşucunun işi.
        .set_runner(|mut app| {
