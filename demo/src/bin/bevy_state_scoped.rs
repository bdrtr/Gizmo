//! # Bevy'nin `ecs/state_scoped` örneğinin Gizmo Engine portu
//!
//! Bir duruma **ait** varlıklar: menü açılınca doğan, menüden çıkınca kendiliğinden silinen
//! nesneler. Bevy bunu iki bileşenle veriyor — `DespawnOnExit<S>` ve `DespawnOnEnter<S>` — ve
//! temizliği motor yapıyor.
//!
//! ([`bevy_states`](../bevy_states/index.html) demosu `State`/`in_state`'in kendisini anlatıyor;
...
//! |------|-------|
//! | `DespawnOnExit<S>` | **yok** |
//! | `DespawnOnEnter<S>` | **yok** |
//! | `OnEnter` / `OnExit` çizelgeleri | **yok** |
//! | ilk duruma giriş de bildirilir | **bildirilmez** — bkz. aşağıdaki tuzak |
//! | `State<S>` + `in_state` | var, ve aynı işi yapıyor |
//!
...
//!
//! Bu, `state_scoped` için tam olarak yanlış tarafa düşen bir tuzak: `OnEnter(A)`'ya bağladığınız
//! kurulum, oyun `A` ile başladığında **hiç koşmuyor**. Demo bunu iki koşuda ölçüyor — biri elle
//! önyükleme yapıyor, öteki yapmıyor.
//!
//! ## Ölçüldü
//!
...
//! İki şey okunuyor:
//!
//! **Sızıntı yok.** `doğan − silinen` her satırda tam 3 — yani elle yazılan temizlik, Bevy'nin
//! `DespawnOnExit`'inin yaptığı işi yapıyor. Yaşayan sayı hiçbir durumda 3'ü aşmıyor.
//!
...
//! güvenen her kurulumun düşeceği tuzak, ve Bevy'de yok — orada `OnEnter(A)` oyun `A` ile
//! başladığında da koşuyor.
//!
...
/// Bevy örneğinin `GameState`'i — `C` yük taşıyor, yani durum bir enum değeri, bir etiket değil.
...
/// `DespawnOnExit<S>`'in elle karşılığı: bu varlık şu duruma ait, o durumdan çıkınca silinsin.
#[derive(Clone, Copy)]
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy State Scoped", 1280, 720)
            ...
            gizmo::egui::Area::new("scoped".into())
                ...
                .show(ctx, |ui| {
                    gizmo::egui::Frame::popup(ui.style()).show(ui, |ui| {
                        ...
                        ui.separator();
                        ui.label("DespawnOnExit / DespawnOnEnter motorda YOK.");
...
fn drive_states(
    ...
    // `apply_transitions` ertelenmiş geçişi uygular ve "oldu mu" bilgisini döndürür. Bevy'nin
    // OnEnter/OnExit'inin yerine geçen tek bilgi bu.
...
/// `DespawnOnExit`'in elle karşılığı: durumu değişince eskiye ait olanı sil, yeniye ait olanı doğur.
///
