//! # Bevy'nin `scene/world_serialization` örneğinin Gizmo Engine portu
//!
...
//!
//! Bevy `#[derive(Reflect)]` + `register_type` ile tipleri tanıtıyor. Gizmo'da karşılığı
//! [`SceneRegistry`] (yani `ComponentRegistry`): bir bileşen orada kayıtlı değilse dosyaya
//! **yazılmaz**. [`default_scene_registry`] motorun kendi bileşenlerini (Transform, RigidBody,
//! Collider, Velocity, Hitbox…) hazır kaydeder; demo kendi bileşenini de aynı yerden ekliyor,
//! Bevy'nin örneğindeki "kendi tipini tanıt" adımının karşılığı bu.
//!
...
/// Demonun kendi bileşeni — Bevy örneğindeki "kendi tipini tanıt" adımının karşılığı.
///
...
fn scene_path() -> String {
    std::env::temp_dir()
        .join("gizmo_bevy_world_serialization.ron")
        .to_string_lossy()
        .into_owned()
}
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy World Serialization", 1280, 720)
