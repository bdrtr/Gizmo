...
//! Bu ikinci çağrı bu demonun asıl konusu: mesh'i değiştirmek burada GPU tamponuna doğrudan
//! yazmak demek.
//!
...
//!
//! **Bir sınır saklanmıyor: motorun prosedürel yolu indeks tamponu almıyor.** Küp bu yüzden
//! **36 köşeye açılıyor** (`from_vertices` üçgen listesi bekliyor); 24 köşe + 36 indeksle
//! kurulamıyor. Aynı geometri, aynı düz gölgeleme — yalnız köşeler paylaşılmıyor.
//!
//! Doku olarak motorun kendi [`AssetManager::create_uv_debug_texture`]'ı kullanıldı: depo hiçbir
//! oyun varlığı taşımıyor ve bu desen zaten "UV nereye düşüyor" sorusunu göstermek için var.
//!
//!
//! **Bir motor tuzağı, ve kolayca gözden kaçıyor:** `add_system` sistemi **sabit
...
/// UV'si elle yazılan mesh'in işaret bileşeni.
#[derive(Clone, Copy)]
struct CustomUv;
...
/// Dönüş hızı — saniyede `1.0 / 1.2` radyan.
const TURN_RATE: f32 = 1.0 / 1.2;

fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Custom Mesh", 1280, 720)
        .with_simple_scene(|scene, state| {
            ...
            // Kamera ve ışık aynı dönüşümü paylaşıyor: (1.8, 1.8, 1.8)'den orijine.
...
/// Altı yüzü elle yazılmış küp — 24 köşe + 36 indeks yerine üçgen listesine açılmış hâli.
///
