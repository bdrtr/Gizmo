...
//!
//! ## Envanter: yedi ayardan ikisi var
//!
//! | yetenek | Gizmo |
//! |---------|-------|
//! | hale şiddeti | [`Renderer::bloom_intensity`] |
//! | parlama eşiği | [`Renderer::bloom_threshold`] |
//! | eşiğin yumuşaklığı | **yok** |
//! | düşük frekans desteği | **yok** |
//! | düşük frekans desteğinin eğriliği | **yok** |
//! | kareye karıştırma kipi (toplamalı / enerji koruyan) | **yok** |
//! | mip sınırı ve ölçek (halenin yarıçapı) | **yok** |
//!
//! Yani "ne kadar" ve "hangi parlaklıktan sonra" söylenebiliyor; halenin biçimi, yumuşaklığı ve
//! kareye nasıl karıştığı söylenemiyor.
//!
//! İkisi de `Renderer`'ın düz alanı, yani `set_render` kapanışından yazılıyor — sistemlerin
//! erişimi yok.
//!
//! ## Bu demonun bulduğu şey: `Material::emissive` ertelenmiş yolda hiçbir şey yapmıyor
//!
...
//! yok**: halenin ölçeği, mip sınırı ve düşük frekans desteği tam olarak o iş için gerekli.
//!
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bloom 3D", 1280, 720)
            ...
            gizmo::egui::Area::new("bloom".into())
                ...
                .show(ctx, |ui| {
                    gizmo::egui::Frame::popup(ui.style()).show(ui, |ui| {
                        ...
                        ui.label("YOK: eşik yumuşaklığı, düşük frekans desteği");
