//! # Bevy'nin `ecs/component_hooks` örneğinin Gizmo Engine portu
//!
//! Bir bileşen bir varlığa **takıldığında, yazıldığında, söküldüğünde** kod çalıştırmak. Bevy'de
...
//! | Bevy kancası | Gizmo |
//! |--------------|-------|
//! | `on_add` | [`World::register_on_add`] |
//! | `on_insert` | **yok** — `on_add` + `on_set` birlikte aynı işi görüyor |
//! | `on_replace` | **yok** |
//! | `on_remove` | [`World::register_on_remove`] |
//! | `on_despawn` | [`World::register_despawn_hook`] — ama **bileşen başına değil, küresel** |
//! | (Bevy'de yok) | [`World::register_on_set`] — her *yazma* |
//!
//! Bevy bileşen başına **tek** kanca alıyor, Gizmo bir `Vec` tutuyor: aynı türe kaç kanca
//! isterseniz. Öte yandan Bevy'nin `on_despawn`'ı bileşene bağlı, Gizmo'nunki dünyaya.
//!
...
//! ## Neden önemli
//!
//! "Bir indeksi bileşenle senkron tut" — Bevy'nin örneğinin gerekçesi bu, ve Gizmo'da da geçerli
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Component Hooks", 1280, 720)
        .with_simple_scene(|scene, state| {
            let tally = Arc::new(Tally::default());

            // Aynı türe kaç kanca isterseniz: Gizmo bir `Vec` tutuyor, Bevy bileşen başına tek
            // kanca alıyor. Burada tür başına birer tane yetiyor.
            for _ in 0..1 {
                let t = tally.clone();
