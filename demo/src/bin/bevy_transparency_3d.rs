//! # Bevy'nin `3d/transparency_3d` örneğinin Gizmo Engine portu
//!
...
//!
//! Bevy `AlphaMode`'u dörde ayırıyor: `Opaque`, `Mask(f32)`, `Blend`, `AlphaToCoverage`.
//! Gizmo'da harmanlama [`Material::is_transparent`], kesme [`Material::alpha_cutoff`];
//! **`AlphaToCoverage` yok** (alfayı MSAA örneklerine dağıtan mod motorda bulunmuyor), o yüzden
//! Bevy'nin o küpünün yerinde burada ikinci bir kesme küpü duruyor.
//!
//! **Kesmenin durumu bu portu yazarken ölçüldü ve tek cümleyle özetlenemiyor:**
//!
//!   * `Material::alpha_cutoff` örnek verisiyle taşınır (`ambient.w`) ve **baked-lit** ile
//!     **ileri PBR** yollarında uygulanır. İkincisi bu portla geldi: ileri shader'ın sabit 0,01
...
//!     çalışıyor, ama bu portun kapsamında bırakılmadı.
...
//! portla birlikte artık gerçekten öyle: alfası düşen bir kesme malzemesi eskiden sıralanan
//! saydam kovaya kayıyordu, şimdi kaymıyor.
//!
//! Sayısal parite notları: zemin 6×6 `srgb(0.3,0.5,0.3)`; kesme küreleri `(±1, 0.5, −1.5)`'te
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Transparency 3D", 1280, 720)
        .with_simple_scene(|scene, state| {
            ...
            // Bevy'de burada AlphaToCoverage küpü var; motorda o mod yok, yerinde ikinci bir
            // kesme küpü duruyor (eşik 0,5) — kesme ile harmanlamanın farkı yan yana görünsün.
            scene.world.spawn_bundle((
...
/// Bevy'nin `fade_transparency`'si: alfa `sin(t)/2 + 0.5` ile 0 ↔ 1 arasında gezer.
///
/// Bevy bütün malzeme varlıklarını dolaşıyor; burada yalnız `Fading` işaretini taşıyanlar
/// değişiyor, çünkü Gizmo'da malzeme varlık değil **bileşen**: her nesnenin kendi kopyası var ve
/// opak olanları da dolaşmak onları da saydamlaştırırdı.
