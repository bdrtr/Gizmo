//! # Bevy'nin `stress_tests/many_cubes` örneğinin Gizmo Engine portu
//!
//! Varlık başına çizim maliyetini ölçmek: bir küre kabuğuna dağıtılmış on binlerce küp, sabit
//! adımlarla dönen bir kamera, ve kare süresi. Bevy'nin örneği bunu komut satırı seçenekleriyle
//! yapıyor; port aynı şeyi çevre değişkenleriyle yapıyor, çünkü demonun `argh` bağımlılığı yok.
//!
...
//!
//! Yani Bevy'nin `--vary-material-data-per-instance` anahtarının burada bir bedeli olmaması
//! gerekir. Demo bunu ölçüyor, üç kip:
//!
...
//! yok — varyantlı olan bir tık daha hızlı bile çıktı, yani parti kesinlikle bölünmüyor.
//! Bevy'nin bu anahtarı için burada bir karşılık aramaya gerek yok: renk zaten örnek
//! tamponunda.
//!
...
//!
//! Kamera **sabit açısal adımla** dönüyor, kare süresiyle değil: Bevy'nin `--benchmark` kipinin
//! karşılığı, ve iki koşuyu karşılaştırılabilir kılan şey bu. Kare süresi motorun kendi
...
//!   cargo run --release -p demo --bin bevy_many_cubes
//! ```
//!
//! ## Kontroller
...
fn main() {
    ...
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Many Cubes", 1280, 720)
        .with_simple_scene(move |scene, state| {
            ...
            for i in 0..cubes {
                ...
                let albedo = if mode == "vary" {
                    // Bevy'nin `--vary-material-data-per-instance`'ı. Anahtarda olmadığı için
                    // partiyi bölmemesi gerekiyor; ölçtüğümüz şey bu.
...
///
/// Dönüş `dt` ile değil sabit bir adımla: Bevy'nin `--benchmark` kipinin karşılığı. Kare süresine
/// bağlı bir kamera, ölçümün kendisini ölçüme sokar.
...
///
/// Bevy'nin varsayılan `Layout::Sphere`'ünün karşılığı.
