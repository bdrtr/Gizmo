...
//!
//! Aydınlatma dersinin ikinci yarısı bu: [`lighting`] ışık **türlerini** yan yana koyuyordu,
...
//! ## Üç bilinçli seçim
//!
//!   * **Küplerin dağılımı** dört satırlık bir xorshift'ten geliyor — deponun
//!     `particles.rs`'inde aynı gerekçeyle yapılanın aynısı: dağılımın
//!     belirleyici olması yeter, kriptografik olması gerekmez, ve demo bir bağımlılık daha
//!     getirmez.
//!   * **Sahneyi gezdiren tuşlar ok tuşları**: basit-sahne kamerası
...
//!   * **Işık şiddeti** fiziksel bir birim değil: motorun `intensity`'si düz bir çarpan, o yüzden
//!     değerler görsel olarak seçildi.
//!
//! Işıkların ucundaki küçük parlayan küreler ışığın **çocuğu**: konumları
...
/// Ok tuşlarıyla gezdirilen zemin ve küplerin işareti.
#[derive(Clone, Copy)]
struct Movable;
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Spotlight", 1280, 720)
...
/// Koniyi salla, açısını da aç-kapa.
///
...
///
/// Dağılımın belirleyici olması yettiği için bu bir bağımlılık gerektirmiyor. Deponun
/// `render/particles.rs`'i de aynı yolu seçmişti.
struct Xorshift(u32);
