...
//! şunlar hedefliyor". İkinci yön bir **ters indeks** demek: hedeften onu hedefleyenlere giden,
//! ilişki kurulup koptukça güncellenmesi gereken eşleme.
//!
//! ## Motorda genel ilişki yok, ama kanca var
//!
//! | yetenek | Gizmo |
//! |---------|-------|
//! | genel ilişki trait'leri (kaynak ve hedef ucu) | **yok** |
//! | ilişki bileşenini bir öznitelikle türetmek | **yok** |
//! | ters indeksi motorun tutması | **yok** |
//! | yerleşik ebeveyn/çocuk ilişkisi | var, ama **sabit**: [`Parent`] / [`Children`], başka ilişki tanımlanamıyor |
//! | ilişki ağacında gezinme yardımcıları | **yok** |
//!
//! Ama ters indeksi **elle kurmanın** aracı motorda var: bileşen yaşam döngüsü kancaları
//! ([`component_hooks`](../component_hooks/index.html) demosunda ölçüldü). Bu demo o indeksi
//! elle ayakta tutuyor — `Targeting` takılınca ters indeksi güncelleyen
//! bir `on_add`, sökülünce temizleyen bir `on_remove`.
//!
//! ## Ve elle kurulan indeksin bir deliği var — ölçülüyor
//!
//! Kancalar her yoldan ateşlenmiyor. `component_hooks` bunu ölçtü: `add_bundle`'ın Table
//! yolu, `spawn_batch`'in ilk varlıktan sonrası ve `clone_entity` **sessiz**. Motorun kendi
//! içinde tuttuğu bir indeks bu yollardan da geçerdi; elle kurulan indeks geçmez.
//!
...
//! arketip sütunlarına yazıyor. `component_hooks`'un tablosunda ölçülen `1/4` oranının
//! buradaki bedeli bu.
//!
//! Delik indeksin **dışarıda** olmasından geliyor: indeksi motor tutsaydı her yol onun içinden
//! geçerdi. Burada ilişki kurmak isteyen bir oyunun ya **yalnız kancanın gördüğü yolları**
//! kullanması, ya da indeksi kancaya değil bir sisteme (her kare tarayan) bağlaması gerekiyor.
//!
...
/// İlişkinin **kaynağı**: bu varlık şu varlığı hedefliyor.
#[derive(Clone, Copy)]
...
/// İlişkinin **tersi**: hedef -> onu hedefleyenler. Motor tutmadığı için burada biz tutuyoruz.
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Relationships", 1280, 720)
        .with_simple_scene(|scene, state| {
            let index: ReverseIndex = Arc::new(Mutex::new(HashMap::new()));

            // Motorun yapmadığı işin elle karşılığı: ters indeksi
            ...
            // B) Kancanın GÖRMEDİĞİ yol: toplu doğurma. Motor içinde tutulan bir indeks bunu da
