//! # Bevy'nin `3d/mesh_ray_cast` + `3d/3d_viewport_to_world` örneklerinin Gizmo portu
//!
//! Bir ışını sahneye atıp neye çarptığını bulmak — ve ışını ekran pikselinden üretmek.
//!
//! ([`bevy_mesh_picking`](../bevy_mesh_picking/index.html) bu parçaları bir seçim akışına
...
//! | Bevy | Gizmo |
//! |------|-------|
//! | `Camera::viewport_to_world` | [`Camera::screen_to_ray`] — birebir |
//! | `MeshRayCast` — **üçgenlere** çarpar | **yok** |
//! | (Bevy'de ayrı) | [`PhysicsWorld::raycast`] — **collider**'a çarpar |
//! | `RayCastSettings` filtreleri | `raycast_filtered` / `raycast_excluding` |
...
//! | sapma | **0,0001°** | 0 |
//!
//! Yani `screen_to_ray` Bevy'nin `viewport_to_world`'ünün yaptığı işi yapıyor, ve doğruluğu
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Mesh Ray Cast", 1280, 720)
