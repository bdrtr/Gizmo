//! # Kol titreşimi
//!
//! Kolu titretmek. Motordaki yolu [`Input::rumble`] / [`Input::rumble_pad`] /
//! [`Input::stop_rumble`], ve arkasında bir istek kuyruğu var.
//!
//! ## İki motor, tek şiddet değil
//!
//! [`RumbleRequest`] iki motoru ayrı istiyor: **`weak`** (küçük, yüksek frekanslı — vızıltı) ve
//! **`strong`** (büyük, alçak frekanslı — gümbürtü). Motorun kendi yorumu adlandırmanın
//! gerekçesini de yazıyor: `left`/`right` demek *bir kolun plastiği* hakkında bir şey söylerdi,
//! oysa istenen şey **hissin kendisi**.
//!
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Gamepad Rumble", 1280, 720)
