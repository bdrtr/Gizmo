//! # Bevy'nin dört çizelge örneğinin Gizmo Engine portu
//!
//! Bu demo dört Bevy örneğini birden karşılıyor, çünkü dördü de aynı şeye — **çizelgenin
//! kendisine** — dokunuyor: `ecs/custom_schedule`, `ecs/custom_executor`,
//! `ecs/nondeterministic_system_order`, `ecs/system_stepping`.
//!
//! ## Motorda çizelgenin neresi açık
//!
//! | Bevy örneği | Bevy'nin yaptığı | Gizmo |
//! |-------------|------------------|-------|
//! | `custom_schedule` | `app.add_schedule(Schedule::new(MyLabel))` | **yok** — [`Phase`] beş varyantlı **kapalı** bir enum |
//! | `custom_executor` | `schedule.set_executor_kind(..)` | **yok** — yürütücü `Schedule::run_batches`, özel ve değiştirilemez |
//! | `nondeterministic_system_order` | kısıtsız sıranın değişkenliğini gösterir | **kısmen** — aşağıda ölçüldü |
//! | `system_stepping` | `Stepping` kaynağı, sistem sistem ilerletme | **yok** — ama `Schedule::run` açık, adımlamayı uygulama kurabilir |
//!
//! ## Ölçülen 1: yığın düzeni deterministik, yığın **içi** değil
//!
//! Bevy'nin `nondeterministic_system_order` örneğinin dersi "kısıt koymazsan sıra değişir".
//! Gizmo'da bu **ikiye ayrılıyor**, ve ayrımı çizelgenin kendi belgesi yazıyor:
//!
...
//! **tek** sıralama görülüyor, sıfır değişim. Yani Gizmo'nun `label`/`after` kısıtları Bevy'nin
//! `.chain()`'inin işini görüyor — sadece elle yazmak gerekiyor, çünkü `.chain()` diye bir
//! kolaylık yok (bkz. `docs/BEVY_PARITY.md` C listesi).
//!
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Schedule Internals", 1280, 720)
