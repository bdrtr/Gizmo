//! # Bevy'nin `ecs/error_handling` örneğinin Gizmo Engine portu
//!
//! Bir sistem hata döndürebilir mi, ve hata olunca ne olur? Bevy'nin cevabı zengin: sistem
//! `Result` döndürebilir, `?` kullanılabilir, ve hatayı ne yapacağını `GLOBAL_ERROR_HANDLER`
//! ya da sistem başına `.map_err(...)` belirler — panikle, günlükle, yok say.
//!
//! ## Motorun cevabı tek kelime: **panikle**
//!
//! | Bevy | Gizmo |
//! |------|-------|
//! | sistem `Result` döndürebilir | **hayır** — `IntoSystem` yalnız `()` döndüren `FnMut` için |
//! | sistemde `?` operatörü | **hayır** |
//! | `GLOBAL_ERROR_HANDLER` | **yok** |
...
//! `Query<Q>`. `Option<Res<T>>` yok — yani Bevy'nin en yaygın "hataya dayanıklı parametre"
//! deseni burada **derlenmiyor**. Ve trait mühürlü olduğu için dışarıdan da eklenemiyor
//! (bkz. [`bevy_ecs_extension_points`](../bevy_ecs_extension_points/index.html)).
//!
...
//! ## Karşılığı ne
//!
//! Bevy'nin `Result` döndüren sistemi Gizmo'da üç parçaya bölünüyor:
//!
...
/// Bevy'nin `Result`'ının yerine geçen defter.
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Error Handling", 1280, 720)
...
/// yazılacak bir yer yok. Bevy'nin `Result` döndüren sistemi orada bir `else` dalı yazabiliyor.
...
fn business_logic(
    ...
    for (id, h) in health.iter() {
        if h.0 < 50.0 {
            // Bevy'de burada `Err(...)` dönerdi ve `?` ile yukarı taşınırdı.
            log.entries
