//! # Bevy'nin `input/gamepad_input` örneğinin Gizmo Engine portu
//!
//! Oyun kolu: çubuklar, tetikler, düğmeler. Bevy'nin örneği bağlı kolun sol çubuğunu ve
//! tetiklerini okuyup konsola basıyor; bu port aynı okumaları yapıyor ve bir küpü sürüyor.
//!
//! **Seride oyun kolunu okuyan ilk demo.**
//!
//! ## Motorun yüzeyi Bevy'ninkinden geniş
//!
//! Bevy `Gamepad` bileşeninden `digital`/`analog` okuyor. Gizmo'nun [`Gamepad`]'i doğrudan
//! adlandırılmış erişim veriyor: [`Gamepad::left_stick`], [`right_stick`](Gamepad::right_stick),
...
//! çubuğun merkezden uzaklığında. Bevy'de ölü bölge oyunun işi.
//!
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Gamepad Input", 1280, 720)
            ...
            gizmo::egui::Area::new("pad".into())
                ...
                .show(ctx, |ui| {
                    gizmo::egui::Frame::popup(ui.style()).show(ui, |ui| {
                        ...
                        ui.label("Bevy'de ölü bölge oyunun işi");
                    });
                });
        })
        .run()
...
/// Kolu okur, küpü sürer, defteri doldurur — Bevy'nin `gamepad_system`'inin karşılığı.
fn read_pad(
