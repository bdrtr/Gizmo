//! # Bevy'nin `animation/animated_transform` örneğinin Gizmo Engine portu
//!
...
//! örneklerinin çoğu deponun taşımadığı bir glTF istiyor, bu örnek ise istemiyor.
//!
//! ## İki modelin karşılaştırması
//!
//! | | Bevy | Gizmo |
//! |---|---|---|
//! | klip | `AnimationClip` + `AnimatableCurve` | [`AnimationClip`] + [`Track`] |
//! | hedef | `AnimationTargetId::from_names([yol…])` — **yolun** karması | [`Track::target_name`] — düz **ad** dizesi |
//! | ne animasyonlanır | `animated_field!(T::alan)` ile herhangi bir animasyonlanabilir alan | yalnız üç kanal: [`Keyframes`]'in `Translation`/`Rotation`/`Scale`'i |
//! | çalıcı | `AnimationGraph` + ağırlıklı düğümler | [`AnimationPlayer`], tek klip |
//! | ara değer | eğri türüne gömülü | [`Interpolation`]: `Linear` / `Step` / `CubicSpline` |
//!
//! **Hedefleme farkı önemli.** Bevy hedefi *yolun* karmasıyla buluyor, yani
//! `planet/orbit_controller/satellite` ile başka bir daldaki `satellite` iki ayrı hedef. Gizmo'nun
//! sistemi kökün altındaki ağacı gezip **yalnız yaprak adına** bakıyor
//! (`track.target_name == name.0`), ve bulduğunu ada göre bir haritaya yazıyor. Aynı adı taşıyan
//! iki varlık aynı hedeftir; ikincisi birincisinin üzerine yazar.
//!
...
//! ## Kanal başına bir iz
//!
//! Bevy tek bir hedefe iki eğri veriyor (uydu hem dönüyor hem ölçekleniyor) ve bu iki ayrı
//! `add_curve_to_target` çağrısı. Gizmo'da da öyle: `Keyframes` kanal başına bir varyant olduğu
//! için iki kanal iki [`Track`] demek, ikisi de aynı `target_name`'i taşıyor. Demo dördünü de
//! kuruyor — Bevy'nin dört eğrisinin birebir karşılığı, aynı zaman damgaları ve aynı değerlerle.
//!
...
/// Bevy'nin adları — hem hiyerarşide hem izlerde aynısı kullanılıyor.
const PLANET: &str = "planet";
const ORBIT: &str = "orbit_controller";
const SATELLITE: &str = "satellite";

fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Animated Transform", 1280, 720)
        .with_simple_scene(|scene, state| {
            ...
            // Bevy'nin hiyerarşisi: gezegen → yörünge denetleyicisi → uydu.
            ...
            // Yörünge denetleyicisi görünmez: işi yalnız çocuğunu döndürmek. Bevy'de de öyle.
            ...
            let satellite = scene.world.spawn_bundle((
                ...
                GlobalTransform::default(),
                // Bevy'nin `Cuboid::new(0.5, 0.5, 0.5)`'i. Motorun `create_cube`'ü boyut
                // parametresi almıyor ve **2 birim** (köşeleri ±1), yani dört kat büyük olurdu;
                // ölçek anahtarlarını bozmamak için küp elle kuruluyor.
                Mesh::from_vertices(device, &cube_vertices(0.5), "bevy_animated_transform::sat"),
...
/// Bevy'nin dört eğrisi, aynı zaman damgaları ve aynı değerlerle.
///
/// Bevy'de uydunun iki eğrisi var (ölçek ve dönüş) ve bu iki ayrı `add_curve_to_target` çağrısı;
/// burada da iki ayrı [`Track`], ikisi de `"satellite"`'i hedefliyor. `Keyframes` kanal başına bir
/// varyant olduğu için başka türlüsü zaten yazılamaz.
