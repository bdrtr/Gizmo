//! # Bevy'nin `ecs/message` örneğinin Gizmo Engine portu
//!
...
//!
//! Bevy'de bir sistem olayı yazar, sıralaması sonraki sistem **aynı karede** okur; örnek zaten
//! `.chain()` ile o sırayı kuruyor. Gizmo'nun [`Events<T>`]'i başka çalışır: yazılan olay o
//! karede **görünmez**, kare sonunda `update()` tamponları döndürür ve olay **bir sonraki**
//! karede okunur.
//!
...
//! sistem sırası olayların görünürlüğünü etkilemez — yani zincir kurmak için `.chain()` gerekmez,
//! ama zincirin gecikmesi vardır.
//!
//! Bevy'nin `MessageMutator`'ı (aynı sistemde hem okuyup hem yazmak, uçuştaki olayı
//! değiştirmek) motorda **yok**: `EventReader`/`EventWriter` ayrıdır. Bu yüzden Bevy'nin
//! "zırhı doğrudan olayın içinden düş" adımı burada "azaltılmış hasarı yeni bir olay olarak
//! yaz" biçiminde yazıldı — aynı sonuç, motorun kendi imkânıyla.
//!
//! ## Kontroller
...
/// Bevy'nin `Timer::from_seconds(1.0, Repeating)`'i.
const TICK_SECONDS: f32 = 1.0;
/// Başlangıç canı.
const MAX_HEALTH: i32 = 100;
/// Her vuruşun ham hasarı (Bevy'de 10).
const HIT: i32 = 10;

fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Events", 1280, 720)
        ...
        // yani hangi sistem önce koşarsa koşsun sonuç aynı. (Bevy'de tersi: orada `.chain()`
        // ile sıralamak gerekiyor, çünkü olay aynı karede okunur.)
...
///
/// Bevy burada `MessageMutator` ile uçuştaki olayı DEĞİŞTİRİYOR; motorda okuma ve yazma ayrı
/// olduğu için azaltılmış hasar ayrı bir tip olarak yazılıyor — ve bu, zinciri okurken daha
/// açık: her aşamanın kendi olayı var.
