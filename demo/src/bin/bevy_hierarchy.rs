//! # Bevy'nin `ecs/hierarchy` örneğinin Gizmo Engine portu
//!
...
//! çocuğa nasıl aktığı ayrı bir demoda: `bevy_parenting`.)
//!
...
//! [`Children`] (`Vec<u32>`).
//!
//! Buradan üç fark çıkıyor:
//!
//! **1. `with_children` yok.** Bevy çocukları ebeveynin doğuşu sırasında bir kapanışta üretiyor.
//! Gizmo'da önce doğuruyor, sonra [`HierarchyExt::add_child`] ile bağlıyorsunuz — Bevy'nin ikinci
//! yöntemi (`commands.entity(parent).add_child(child)`) ise birebir var.
//!
...
//! varlığı gösterir. Bevy'nin `Children`'ı `Entity` tutar ve bu sorunu taşımaz.
//!
//! **3. Ve asıl ölçüm: düz `despawn` ebeveynin listesini temizlemiyor.** Bevy'nin örneği bir
//! çocuğu `commands.entity(child).despawn()` ile yok ediyor ve ebeveynin listesinin kısalmasını
//! bekliyor. Gizmo'da bu bakım [`HierarchyExt::remove_child`]'ın işi; düz `despawn` çağrısı
//! ebeveynde **asılı bir kimlik** bırakıyor.
//!
//! ## Ölçüldü (2026-08-23)
//!
...
/// Ağaçtaki her küpün işareti — Bevy'nin `With<Sprite>` filtresinin karşılığı.
#[derive(Clone, Copy)]
struct Node;
...
/// Kökün rengi, ve iki çocuğun (Bevy'de mavi ile yeşil).
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Hierarchy", 1280, 720)
...
/// Bevy'nin `rotate`'i: kök bir yöne, çocuklar öbür yöne döner.
///
/// Bevy çocukları ebeveynin [`Children`] listesinden geziyor. Burada da öyle — ama liste `u32`
/// tuttuğu için önce yaşayan varlığa çevrilmesi gerekiyor, ve **ölü bir kimlik sessizce
/// atlanıyor**. Demonun ikinci aşamasında listede tam olarak öyle bir kimlik var.
fn rotate(
...
fn advance_stages(world: &mut gizmo::core::World) {
    ...
    match stage {
        // 2 s: son çocuğu DÜZ despawn ile öldür — Bevy'nin yaptığı çağrının karşılığı.
        0 if elapsed >= 2.0 => {
...
/// Kök + iki çocuk. Bevy'nin `setup`'ının karşılığı.
fn build_tree(world: &mut gizmo::core::World, cube: &Mesh, template: &Material) {
    ...
    // Bevy'nin `with_children` kapanışının karşılığı: doğur, sonra bağla.
    let child_a = spawn(world, Vec3::new(3.0, 0.0, 0.0), 0.75, CHILD_A);
    world.add_child(root, child_a);
    // Bevy'nin ikinci yöntemi — bunun birebir karşılığı var.
