//! # Bevy'nin `ecs/observer_propagation` örneğinin Gizmo Engine portu
//!
//! Bir olayın hiyerarşide **yukarı kabarması** (bubbling): yaprakta tetiklenen olay, ata zincirini
//! tırmanarak her katmandaki dinleyiciyi çalıştırır. Bevy'nin örneği bunu bir zırh/gövde/oyuncu
//! zincirinde gösteriyor — hasar zırha iniyor, oyuncuya kadar çıkıyor.
//!
//! ## Motorda kabarma **var** — ve üç sınırı var
//!
//! | Bevy | Gizmo |
//! |------|-------|
//! | `#[entity_event(propagate)]` | [`EntityEvent::can_propagate`] -> `true` |
//! | `On<E>` dinleyici, `world.observe(entity, ..)` | aynı |
//! | `trigger` ile gönderme | aynı |
//! | **yolu seçmek** (`Traversal` türü) | **yok** — yol her zaman `Parent` zinciri |
//! | **kabarmayı durdurmak** (`propagate(false)`) | **yok** — hiçbir dinleyici yürüyüşü kesemiyor |
...
//!
//! Bevy'nin örneğinin can alıcı satırı tam da bu — orada bir dinleyici olayı yakalayıp yukarı
//! çıkmasını engelleyebiliyor. Gizmo'da o desen **yazılamıyor**; yapılabilecek şey olayın
//! *etkisini* bir bayrakla bastırmak, ama dinleyiciler yine de çalışıyor.
//!
//! ## Ölçüldü
//!
...
//! orada biterdi. Bitmiyor — ondan sonra **2 halka daha** (1 ve 0) ziyaret ediliyor. Bevy'de bu
//! sayı `propagate(false)` ile 0 olurdu.
//!
...
/// Kabaran olay — Bevy'nin `#[entity_event(propagate)]`'inin karşılığı.
#[derive(Clone, Copy)]
struct Damage {
    target: Entity,
    amount: f32,
...
struct PropReport {
    ...
    veto_at: usize,
    /// Veto denemesinden SONRA kaç halka daha ziyaret edildi — Bevy'de 0 olurdu.
    after_veto: usize,
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Observer Propagation", 1280, 720)
        .with_simple_scene(|scene, state| {
            ...
            for (depth, entity) in chain.iter().enumerate() {
                ...
                scene.world.observe::<Damage, _>(*entity, move |on: On<Damage>| {
                    ...
                    // Bevy'de burada `on.propagate(false)` yazılabilirdi ve yürüyüş dururdu.
                    // Gizmo'da öyle bir çağrı YOK: `On` değerle geliyor, dönüş `()`, ve
                    // `trigger` döngüsü dinleyicinin ne yaptığına bakmıyor.
