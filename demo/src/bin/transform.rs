...
//!
//! ## Bu demo bir eksik buldu — ve o eksik artık kapalı
//!
//! Sahne neredeyse tamamen `transform.forward()`, `transform.up()` ve
//! `transform.looking_at(hedef, yukarı)` üzerine kurulu. Demo yazıldığında Gizmo'nun
...
//! `Mat4::look_at_rh` ve `Vec3::NEG_Z` kullanıyor, yani **sağ elli, ileri −Z**.
//!
//! `looking_at` dönüşü **uygulamıyor, döndürüyor** — çünkü buradaki iş tam olarak onu
//! harmanlamak (`rotation.lerp(hedef, ağırlık)`), ve doğrudan uygulasaydı o imkân elden giderdi.
//!
//! ## İkinci nokta: zincir yok, sıralama etiketle kuruluyor
//!
//! Motorda sistemleri tek çağrıyla birbirine bağlayan bir zincir **yok**; sıralama
...
/// Merkezdeki kürenin ölçek sınırları.
#[derive(Clone, Copy)]
struct Center {
    max_size: f32,
    min_size: f32,
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Transform", 1280, 720)
        ...
        // Zincir yerine iki kenar: rotate → move → scale.
...
/// Küpü her kare biraz küreye doğru çevirir.
///
...
fn rotate_cube(
    ...
    // Dönülecek nokta: kürelerin konumlarının toplamı (sahnede tek küre var).
...
/// Küreyi, küpün başlangıcından ne kadar uzaklaştığına göre küçültür. Ayrıca ölçüm defterini
/// doldurur.
fn scale_center(
