//! # Bevy'nin `3d/irradiance_volumes` + `3d/light_probe_blending` örneklerinin Gizmo portu
//!
...
//!
//! ## Bevy'ye göre eksik olanlar
//!
//! | Bevy | Gizmo |
//! |------|-------|
//! | `IrradianceVolume` bileşeni, render'a bağlı | **yok** — `ProbeGrid` elle sürülüyor |
//! | voxel dokusundan GPU örneklemesi | **yok** — örnekleme CPU'da |
//! | `LightProbe` bileşeni + otomatik seçim | **yok** |
...
//! `ProbeGrid::sample` + `SHCoeffs::evaluate` Bevy'nin prob karışımının yaptığı işi yapıyor —
//! **CPU'da**.
//!
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Irradiance Volumes", 1280, 720)
