//! # Bevy'nin `dev_tools/infinite_grid` örneğinin Gizmo Engine portu
//!
//! Editör benzeri uygulamaların zemini: ufka kadar giden, ölçek duygusu veren bir ızgara. Bevy'de
//! bir eklenti takıp `InfiniteGrid` bileşenli bir varlık doğuruyorsunuz, gerisi hallediliyor.
//!
//! ## Motorda ızgara **var**, ama oyun yolu onu çizmiyor
//!
//! Bu portun asıl bulduğu şey bu, ve uydurma değil — motorun kendi `routing.rs`'i yazıyor:
//!
...
//!
//! ## İkinci fark: `Visibility` bileşeni yok
//!
//! Bevy'nin örneğindeki tek sistem `toggle_visibility` — `Visibility::Hidden` yazıyor. Gizmo'da
//! böyle bir bileşen **yok**; `Visibility` adında bir tip var ama o kare başına hesaplanan frustum
//! sonucudur, kullanıcı bileşeni değil. Görünürlüğü kapatmanın motordaki karşılığı
//! [`ShadowCasting::Only`]: nesne çizilmez ama **gölgesini düşürmeye devam eder**. Yani Bevy'nin
//! "gizle"si ile motorun karşılığı birebir aynı şey değil, ve demo bunu ekranda gösteriyor —
//! zemin kaybolur, gölgesi durur.
//!
//! ## Kontroller
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Infinite Grid", 1280, 720)
        .with_simple_scene(|scene, state| {
            ...
            // Bevy'nin `InfiniteGrid` varlığının karşılığı: motorun ızgara malzemesini taşıyan
            // bir zemin. Oyun yolunda ne olduğunu görmek için burada.
            ...
            // Bevy'nin iki yarı saydam küpü: biri ızgaranın üstünde, biri altında. Alttaki,
            // zeminin arkasından görünüp görünmediğini söyler.
...
/// `Only` "çizme ama gölge düşürmeye devam et" demek — Bevy'nin `Hidden`'ının tam karşılığı
/// değil, ve fark ekranda görünüyor.
fn toggle_grid(
...
fn draw_line_grid(
    ...
    for i in -CELLS..=CELLS {
        ...
        // Ana eksenler daha parlak — Bevy'nin ızgarası da x/z eksenlerini ayırıyor.
