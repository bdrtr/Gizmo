//! # Bevy'nin `ecs/removal_detection` örneğinin Gizmo Engine portu
//!
//! Bir bileşen bir varlıktan koptuğunda haberdar olmak. Bevy'de bu üç satır: `add_observer` ile
//! `On<Remove, MyComponent>` dinlersiniz, bileşen kalkınca gözlemci çağrılır ve örnek sprite'ı
//! camgöbeğine boyar.
//!
//! ## Gizmo'da gözlemci **var**, ama yalnız ekleme için
//!
//! [`World::add_observer`] motorda mevcut — ve imzası `On<Insert, T>`'e sabitli. Yani Bevy'nin
//! bu örneğindeki `On<Remove, …>` gözlemcisinin doğrudan karşılığı **yok**. Kaldırmanın kapısı
...
//!
//! ## Portun asıl ölçtüğü şey: hangi yol kancayı tetikliyor, hangisi sessiz
//!
...
//! camgöbeğine dönüyor (Bevy'nin `srgb(0.5, 1, 1)`'i) ve HUD "çalıştı" yazıyor.
//!
...
/// Bevy'nin `MyComponent`'i — kaldırılacak olan. **Table** depolamalı (varsayılan).
#[derive(Clone, Copy)]
struct MyComponent;
...
/// Kanca çalışınca küpün aldığı renk — Bevy'nin `srgb(0.5, 1.0, 1.0)`'i.
...
fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy Removal Detection", 1280, 720)
        .with_simple_scene(|scene, state| {
            ...
            // Kancanın kendisi: Bevy'nin `react_on_removal` gözlemcisinin karşılığı. `&mut World`
...
/// Bevy'nin `react_on_removal`'ı: kanca çalışınca küpü camgöbeğine boyar ve deftere işler.
///
/// Kancanın imzası `(&mut World, Entity)` — Bevy'nin `On<Remove, T>` olayının taşıdığı varlık
