//! # Bevy'nin `3d/3d_shapes` örneğinin Gizmo Engine portu
//!
//! Şekil ilkelleri: matematiksel tanımdan mesh üretmek. Bevy'nin örneği 26 mesh diziyor ve
//! hepsine çalışma anında üretilmiş bir UV hata ayıklama dokusu giydiriyor.
//!
//! ## Sayılan envanter: 26 şeklin 6'sının motorda hazır yapıcısı var
//!
//! Bevy'nin listesi üç kümeden oluşuyor — 11 katı şekil, 8 ekstrüzyon (2B şekli derinlik vererek
//! hacme çıkarmak) ve 7 halka ekstrüzyonu. Motorun `AssetManager`'ında karşılığı olanlar:
//!
//! | Bevy                       | Gizmo                        |
//! |----------------------------|------------------------------|
//! | `Cuboid`                   | `create_cube`                |
//! | `Capsule3d`                | `create_capsule`             |
//! | `Torus`                    | `create_torus`               |
//! | `Cylinder`                 | `create_cylinder`            |
//! | `Cone`                     | `create_cone`                |
//! | `Sphere` (uv)              | `create_sphere`              |
//!
...
//! geliyor. Geri kalan beşi ayrı ayrı eksik: `Tetrahedron`, `ConicalFrustum`, ikoza küre,
//! `Segment3d` ve `Polyline3d`.
//!
...
//! **kesik koni** (`ConicalFrustum` — silindirin iki farklı yarıçaplısı) ve **beşgenin
//! ekstrüzyonu** (Bevy'nin `Extrusion::new(RegularPolygon, 1.0)`'unun karşılığı, iki kapak +
//! yan yüzler). Üçü de üst sırada, motorun hazır altısı alt sırada duruyor.
//!
//! Bir şey **birebir** eşleşiyor: Bevy'nin çalışma anında ürettiği UV dokusunun karşılığı
//! motorda hazır — [`AssetManager::create_uv_debug_texture`].
//!
//! ## Kontroller
//!   * **R** — dönmeyi durdur/başlat (Bevy'de de R)
...
/// Dönen şekillerin işareti — Bevy'nin `Shape`'i.
#[derive(Clone, Copy)]
struct Shape;
...
/// Sıraların yatay yayılımı — Bevy'nin `SHAPES_X_EXTENT`'i.
const X_EXTENT: f32 = 12.0;

fn main() {
    App::<SimpleSceneState>::new("Gizmo Engine - Bevy 3D Shapes", 1280, 720)
        .with_simple_scene(|scene, state| {
            ...
            let handmade: [Mesh; 3] = [
                Mesh::from_vertices(device, &tetrahedron(0.9), "bevy_3d_shapes::tetrahedron"),
                Mesh::from_vertices(
                    device,
                    &conical_frustum(0.7, 0.3, 1.2, 24),
                    "bevy_3d_shapes::conical_frustum",
                ),
                Mesh::from_vertices(
                    device,
                    &extruded_polygon(0.7, 5, 1.0),
                    "bevy_3d_shapes::extruded_pentagon",
                ),
            ];
        ...
        .set_ui(|world, _state, ctx| {
            ...
            gizmo::egui::Area::new("shapes".into())
                ...
                .show(ctx, |ui| {
                    ...
                    ui.separator();
                    ui.label("Bevy'nin listesi 26 mesh:");
...
/// Bevy'nin `rotate`'i — R ile durdurulabiliyor.
fn rotate(
...
/// Bevy'nin `Tetrahedron`'u: dört eşkenar üçgen.
...
/// Bevy'nin `ConicalFrustum`'u: alt ve üst yarıçapı farklı bir silindir.
///
...
/// Bevy'nin `Extrusion::new(RegularPolygon, depth)`'unun karşılığı: bir düzgün çokgeni derinlik
/// vererek hacme çıkarmak.
///
/// **Motorda böyle bir makine yok** ve Bevy'nin 26 girdisinin 15'i tam olarak bundan geliyor —
