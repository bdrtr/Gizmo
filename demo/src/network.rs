...
use gizmo_net::protocol::{ClientChannel, ClientMessage, ServerChannel, ServerMessage};
...
pub fn client_network_system(world: &mut World, dt: f32) {
    ...
    if let Some(input) = world.get_resource::<gizmo::core::input::Input>() {
        let x = if input.is_key_pressed(gizmo::winit::keyboard::KeyCode::KeyD as u32) {
            1.0
        } else {
            0.0
        ...
        let z = if input.is_key_pressed(gizmo::winit::keyboard::KeyCode::KeyW as u32) {
            -1.0
        } else {
            0.0
        ...
        if x != 0.0 || z != 0.0 || jump {
            commands.push(ClientMessage::PlayerInput {
                move_x: x,
                move_z: z,
                jump,
            });
        ...
        if client_res.client.is_connected() {
            ...
            for cmd in commands {
                if let Ok(encoded) = bincode::serialize(&cmd) {
                    client_res
                        .client
                        .send_message(ClientChannel::Command, encoded);
                }
            }
