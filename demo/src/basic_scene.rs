...
pub fn setup_basic_scene(
    ...
    world.add_component(floor_entity, gizmo::renderer::asset::AssetManager::create_plane(&renderer.device, 500.0));
    let mut floor_mat = Material::new(base_tbind.clone()).with_pbr(Vec4::new(0.1, 0.1, 0.15, 1.0), 0.9, 0.1);
    ...
    for i in 0..40 {
        ...
        if rx > -20.0 && rx < 20.0 { rx += 40.0; }
        if rz > -20.0 && rz < 20.0 { rz += 40.0; }

        let b_ent = world.spawn();
        world.add_component(b_ent, Transform::new(Vec3::new(rx, h / 2.0, rz)).with_scale(Vec3::new(w, h, d)));
        world.add_component(b_ent, cube_mesh.clone());
        
        let color = if i % 3 == 0 { Vec4::new(0.1, 0.8, 1.0, 1.0) } else { Vec4::new(0.2, 0.2, 0.25, 1.0) };
        world.add_component(b_ent, Material::new(base_tbind.clone()).with_pbr(color, 0.5, 0.8));
    ...
    world.add_component(xaxis, Transform::new(Vec3::new(axis_length / 2.0, 0.1, 0.0)).with_scale(Vec3::new(axis_length, axis_thickness, axis_thickness)));
    world.add_component(xaxis, gizmo::renderer::asset::AssetManager::create_cube(&renderer.device));
    world.add_component(xaxis, Material::new(base_tbind.clone()).with_unlit(Vec4::new(1.0, 0.0, 0.0, 1.0)));
    ...
    world.add_component(yaxis, Transform::new(Vec3::new(0.0, axis_length / 2.0, 0.0)).with_scale(Vec3::new(axis_thickness, axis_length, axis_thickness)));
    world.add_component(yaxis, gizmo::renderer::asset::AssetManager::create_cube(&renderer.device));
    world.add_component(yaxis, Material::new(base_tbind.clone()).with_unlit(Vec4::new(0.0, 1.0, 0.0, 1.0)));
    ...
    world.add_component(zaxis, Transform::new(Vec3::new(0.0, 0.1, axis_length / 2.0)).with_scale(Vec3::new(axis_thickness, axis_thickness, axis_length)));
    world.add_component(zaxis, gizmo::renderer::asset::AssetManager::create_cube(&renderer.device));
    world.add_component(zaxis, Material::new(base_tbind.clone()).with_unlit(Vec4::new(0.0, 0.0, 1.0, 1.0)));
    ...
    world.add_component(skybox, Material::new(base_tbind.clone()).with_unlit(Vec4::new(0.05, 0.05, 0.1, 1.0)));
    ...
    world.add_component(radio_entity, Transform::new(Vec3::new(10.0, 1.0, 0.0))
        .with_scale(Vec3::new(1.5, 1.5, 1.5))); // Havada süzülen hafifçe büyük bir obje
        
    world.add_component(radio_entity, gizmo::renderer::asset::AssetManager::create_cube(&renderer.device));
    
    // Altın sarısı/fosforlu radyo materyali
    let radio_mat = gizmo::renderer::components::Material::new(base_tbind.clone())
        .with_pbr(Vec4::new(1.0, 0.8, 0.0, 1.0), 1.0, 0.2); // Çok parlak, metalik
    world.add_component(radio_entity, radio_mat);
    world.add_component(radio_entity, gizmo::renderer::components::MeshRenderer::new());
    world.add_component(radio_entity, EntityName("Radyo (Boombox)".to_string()));
    
    // AudioSource Bileşeni: max_distance ayarlı, loop aktif
    world.add_component(radio_entity, gizmo::audio::AudioSource::new("music")
        .with_loop(true)
    ...
    world.add_component(car, Material::new(base_tbind.clone()).with_unlit(Vec4::new(0.0, 0.5, 1.0, 1.0))); // Mavi Taksi
    ...
    for i in 0..15 {
        ...
        world.add_component(coin, Transform::new(Vec3::new(rx, 1.0, rz)).with_scale(Vec3::splat(1.5)));
        world.add_component(coin, gizmo::renderer::asset::AssetManager::create_sphere(&renderer.device, 0.5, 16, 16));
        world.add_component(coin, Material::new(base_tbind.clone()).with_unlit(Vec4::new(1.0, 0.8, 0.0, 1.0)));
        world.add_component(coin, gizmo::renderer::components::MeshRenderer::new());
        world.add_component(coin, gizmo::renderer::components::PointLight::new(Vec3::new(1.0, 0.8, 0.0), 3.0));
