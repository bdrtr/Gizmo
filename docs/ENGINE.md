...
| **`missing_docs`** | **ON per crate, as a ratchet.** All 10 Stage A crates plus `gizmo-window`, `gizmo-ui` and — since 2026-08-17 — `gizmo-app` are at zero and warn (CI's `-D warnings` makes that a deny). Re-measured 2026-08-17, and the table was accurate: renderer 874, editor 281, facade 168, scripting 102, studio 40, analysis 33. `gizmo-app`'s 23 were the windowed and headless builders — `new`, `set_setup`, `set_update`, `add_system` and the rest of what every demo calls — so they were also the crate's whole docs.rs front page. A crate joins the ratchet when it reaches zero; that rule is what stops the number growing while the backlog is worked off. |
