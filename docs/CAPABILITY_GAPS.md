...
| Observer propagation control | Bubbling works (measured `[4, 3, 2, 1, 0]` from leaf to root), but **no listener can cancel the walk** — measured 2 more links visited after one tried — and the path is always the `Parent` chain. **Still open after the world handle landed (2026-08-24), and worth saying why:** what is missing is not access but a *return channel*. `On` arrives by value and the listener returns `()`, so the dispatch loop has nowhere to read an answer; a listener now holding `&mut World` can do anything except tell the walk to stop | `observer_propagation` |
