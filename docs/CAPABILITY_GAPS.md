...
| `iter_combinations` | **Present since 2026-08-23**, read-only: every unordered pair once, from `&self` with `Q: ReadOnlyQuery`. No `_mut` — two `&mut` into one storage is sound only because `i != j`, which the borrow checker cannot see, so it needs `unsafe`; a writing loop reads pairs and applies them in a second pass. That second pass is still forced by an older constraint: a query holding `Mut<T>` cannot be iterated read-only at all (`iter` wants `ReadOnlyQuery`). Measured with both loops side by side: **66 pairs each and a largest acceleration difference of exactly 0.000000000** over 1500 frames — same pairs, same order, same float ops. Correctness also verified by conservation: total momentum stayed under 1e-5 | `iter_combinations` |
