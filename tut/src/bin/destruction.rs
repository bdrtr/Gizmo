...
fn run() {
    App::<DestructionGame>::new("Gizmo — Voronoi Destruction Demo", 1600, 900)
        .set_setup(|world, _renderer| {
            ...
            game.default_mat = Some(gizmo::renderer::components::Material::new(bg).with_pbr(Vec4::new(0.6, 0.6, 0.6, 1.0), 0.5, 0.0));
        ...
        .set_update(|_world, state, dt, input| {
            ...
            if input.is_mouse_button_just_pressed(0) {
                ...
                state.pending_balls.borrow_mut().push((state.cam_pos, forward * speed));
            }
        ...
        .set_render(|world, state, encoder, view, renderer, _light_time| {
            ...
            if !state.pending_chunks.borrow().is_empty() {
                ...
                for chunk in chunks.drain(..) {
                    ...
                    let e = world.spawn();
                    world.add_component(e, Transform::new(chunk.center_of_mass + Vec3::new(0.0, 25.0, 0.0))); // bina center'i
                    ...
                    world.add_component(
                        ...
                        gizmo::renderer::components::Mesh::new(
                            ...
                            Vec3::ZERO,
                            "voronoi_chunk".into(),
                            gizmo::math::Aabb::new(Vec3::ZERO, half_extents)
                        )
                    );
                    
                    world.add_component(
                        e,
                        state.default_mat.clone().unwrap()
                    );
            ...
            if !state.pending_balls.borrow().is_empty() {
                ...
                for (pos, vel) in balls.drain(..) {
                    ...
                    let ball_id = {
                        ...
                        cmd.spawn_rigid_sphere(pos, radius, gizmo::color::Color::RED, mass).id()
                    };
