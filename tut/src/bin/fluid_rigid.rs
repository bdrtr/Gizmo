...
fn main() {
    App::<FluidRigidDemo>::new("Gizmo — Faz 7.1 Fluid-Rigid Coupling", 1600, 900)
        .set_setup(|world, _renderer| {
            ...
            // Kamera
            let cam = world.spawn();
            world.add_component(cam, Transform::new(state.cam_pos).with_rotation(pitch_yaw_quat(state.cam_pitch, state.cam_yaw)));
            world.add_component(cam, Camera::new(std::f32::consts::FRAC_PI_3, 0.1, 500.0, state.cam_yaw, state.cam_pitch, true));
            state.cam_id = cam.id();

            // Güneş
            let sun = world.spawn();
            world.add_component(sun, Transform::new(Vec3::new(0.0, 10.0, 5.0)).with_rotation(Quat::from_axis_angle(Vec3::X, -0.8)));
            world.add_component(sun, DirectionalLight::new(Vec3::new(1.0, 0.95, 0.9), 3.5, gizmo::renderer::components::LightRole::Sun));
            ...
            for i in 0..5 {
                ...
                world.add_component(box_ent, Transform::new(Vec3::new(0.0, 2.0 + (i as f32) * 1.5, 0.0)));
